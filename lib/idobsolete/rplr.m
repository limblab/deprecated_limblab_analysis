...
if ~any(strncmpi(adm,{'ff','kf','ng','ug'},2))
    ctrlMsgUtils.error('Ident:estimation:recurCheck1','rplr')
end
adm = lower(adm(1:2));
...
if ns==1
    if length(nn)~=2
        ctrlMsgUtils.error('Ident:estimation:rplrTimeSeries')
    end
end
if ns==2
    if length(nn)~=6
        ctrlMsgUtils.error('Ident:estimation:rplrOrders')
    end
end
if ns==2
    na=nn(1);nb=nn(2);nc=nn(3);nd=nn(4);nf=nn(5);nk=nn(6);
else
    na=nn(1);nc=nn(2);nk=1;nb=0;nd=0;nf=0;
end
if nk<1
    ctrlMsgUtils.error('Ident:estimation:recurCheck2','rplr')
end
d=na+nb+nc+nd+nf;
if ns>2
    ctrlMsgUtils.error('Ident:estimation:recurMultiInput','rplr')
end
...
tid=na+nb+nc+1:na+nb+nc+nd;
tif=na+nb+nc+nd+1:d;
ia=tia;ib=na+nk:na+nb+nk-1;
ic=tic+nk-1;
id=tid+nk-1;
iff=tif+nk-1;
iib=na+1:na+nb+nk-1;
...
if length(th0)~=d
    ctrlMsgUtils.error('Ident:estimation:recurCheck3','rplr(Z,NN,adm,adg,th0,...)')
end
...
if adm(1)=='k'
    [sR1,SR1]=size(adg);
    if sR1~=d || SR1~=d
        ctrlMsgUtils.error('Ident:estimation:recurCheck4','rplr(Z,NN,''kf'',R1,...)')
    end
    R1=adg;lam=1;
end
if adm(2)=='g'
...
for kcou=1:nz
    ...
    if nc>0
        phi(na+nbm+1)=epsilon;
    end
    if nd>0
        phi(na+nbm+nc+1)=-v;
    end
    if nf>0
        phi(na+nbm+nc+nd+1)=-w;
    end
