...
%GETNNPAR  Parameter vector for IDARX model.

%   Copyright 1986-2012 The MathWorks, Inc.

if ~isnumeric(A1) || ~isnumeric(B1)
   ctrlMsgUtils.error('Ident:idmodel:idarxNonNumericPar')
end
...
if ra~=ny || rb~=ny,
   ctrlMsgUtils.error('Ident:idmodel:idarxWrongParSize1')
end
if floor(na)~=na
   ctrlMsgUtils.error('Ident:idmodel:idarxWrongParSize2','A','A')
elseif floor(nb)~=nb
   ctrlMsgUtils.error('Ident:idmodel:idarxWrongParSize2','B','B')
end
for outp=1:ny
   for kk=1:ny
      sl=A(outp,ny+kk:ny:na*ny+ny);
      sl=(sl~=0);sl=sl(length(sl):-1:1);
      sl=cumsum(sl);ksl=find(sl==0, 1, 'last' );
      if isempty(ksl),ksl=0;end
   ...
   for kk = 1:nu
      ...
      sl1 = (sl~=0);
      ksl = find(cumsum(sl1)==0, 1, 'last' );
      if isempty(ksl), ksl = 0; end
      nnk(outp,kk) = ksl;
      sl1 = sl1(length(sl1):-1:1); sl = cumsum(sl1);
      ksl = find(sl==0, 1, 'last' );
      if isempty(ksl), ksl=0; end
      if ksl==nb+1
         nnb(outp,kk) = 0; 
      else
