...
%IDHELP  Micro-manual for navigation within System
...
%        frequency domain data. Type IDHELP IDFRD for more on frequency
%        response data.
%
%   MODELSTRUCTURE is a variable that defines the structure of the model.
%        The toolbox supports many kinds of linear and nonlinear models.
%        Type IDHELP LINMODEL for an overview of linear models.
%        Type IDHELP NLMODEL for an overview of nonlinear models.
%
%   MODEL is the resulting model, in formats that depend on the structure.
%        Type IDHELP EVALUATE for information on how to evaluate and use
%        the model.
%
...
%
%   Type:
%       IDHELP to display this help.
%       IDHELP DATA for information on data objects used by the toolbox.
%       IDHELP FRD for information on frequency response representation
%                    used by the toolbox.
%       IDHELP LINMODEL for an overview of linear models.
%       IDHELP NLMODEL for an overview of nonlinear models.
%       IDHELP PROCESS for information on process models.
%       IDHELP IOBB for information on linear input-output black models.
%       IDHELP SSBB for information on state-space black box models.
%       IDHELP SSCT for information on continuous-time black box models.
%       IDHELP CTM for more information on estimation of continuous-time models.
%       IDHELP SSSTRUC for information state-space models with internal
%                      structure specified by user.
%       IDHELP EVALUATE for information on how to use models.
%       IDHELP CHANNELS for information on input and output channel
%                       management in data and model objects.
%       IDHELP ADVICE for ideas to get started.
%       IDHELP NOTE points to a few things to note.

%   Copyright 1986-2011 The MathWorks, Inc.

disp('IDHELP is obsolete. Refer to the System Identification Toolbox documentation for all information.')
return
...
if nargin==0
   ...
   return
elseif ~ischar(type)
   ctrlMsgUtils.error('Ident:general:idhelp1')
end
...
switch lower(type(1:min(3,end)))
   ...
   case 'iob'
      ...
      DisplayStr = {...
         ...
         ' BoxJenkins:  y(t) = [B(q)/F(q)]u(t-nk) + [C(q)/D(q)]e(t)'...
   ...
   case 'ssc'
      DisplayStr = {...
         ...
         ' '...
         ' 1. Estimate a discrete-time model (type IDHELP IOBB or IDHELP SSBB)'...
         '    and convert to continuous time by MODELC = D2C(MODEL).'...
         ' '...
         ' 2. Estimate a state-space continuous time model in canonical' ...
         '    form by'...
         '    MODEL = PEM(DATA,nx,''nk'',nk,''Ts'',0,''SSpar'',''Can'')'...
         ' '...
         '    The optional P/V pair ''nk'',nk determines whether a D-matrix in'...
         '    the state space model is estimated. nk is a 1-by-nu row vector (nu'...
         '    being the number of inputs) of zeros and ones. nk(ku) = 0 means that a'...
         '    direct term from input number ku is estimated, i.e. the ku:th column'...
         '    of D. nu(ku) = 1 means that this column is fixed to zero. Default is'...
         '    nk = ones(1,nu).'...
         };
   case 'sss'
   ...
   case 'eva'
      DisplayStr={...
         ...
         ' SIM(MODEL,U): Simulates the model with a given input U.'...
         ' PREDICT(MODEL,DATA,K): Predicts future outputs using the model.'...
         ' COMPARE(DATA,MODEL): Compares the measured model output in DATA with the model'...
         '                      response.'...
         ' STEP(MODEL): Plots the step response.'...
         ' RESID(MODEL,DATA):   Computes and analyses the model residuals for the DATA.'...
         ' PLOT(MODEL): Shows certain characteristics of the model. (Requires the Control'...
         '              System Toolbox for linear models.'...
         ' '...
         ' All these commands apply to any kind of models. For linear models also the'...
         ' following commands apply:'...
         ' IMPULSE(MODEL): Plots the impulse response of the model.'...
         ' BODE(MODEL), NYQUIST(MODEL): Plots the Bode and Nyquist diagrams of the model.'...
         ' PZMAP(MODEL):       Plots zeros and poles of the model.'...
         ''...
         ' In most of these commands, several models can be compared by just giving more'...
         ...
         ' '...
         ' To compute linear model representations in different forms, use'...
         ' '...
         ' ZPKDATA(Model), TFDATA(Model), SSDATA(Model), POLYDATA(Model)'...
         ' '...
         ' If you have Control System Toolbox, any model from System Identification'...
         ' Toolbox can be converted to any LTI-object by TF(Model), SS(Model), ZPK(Model),'...
         ' and FRD(Model). VIEW(Model) or PLOT(Model) will then invoke the LTI-viewer.'...
         ' '...
         };
   case 'cha'
      DisplayStr ={...
         ...
         ' '...
         ' For linear IDMODELS, MODEL(Outputs,Inputs) extract submodels in a similar fashion:'...
         ...
         ''...
         ' For nonlinear IDNLMODELS, the channel selection is not fully supported.'};
   ...
   case 'dat'
      DisplayStr ={...
         ...
         ' OutputName: String. For multiputput, use cell arrays, e.g. {''Speed'',''Voltage''}'...
   ...
   case 'lin'
      DisplayStr ={...
         ...
         ''...
         ' Detailed information is found under IDHELP PROCMOD, IOBB, SSBB, SSSTRUC, CTMOD or SSCT.'...
         ''...
   ...
   case 'nlm'
      DisplayStr={...
         ' NONLINEAR MODELS:'...
         ' Three types of nonlinear models are supported: (y is output and u is input.)'...
         ''...
         ' 1. Nonlinear ARX models: These are discrete time predictor models that model'...
         ' future outputs as quite arbitrary nonlinear mappings from past input-output data:'...
         ' '...
         ' y(t) = F(y(t-1),..y(t-Na),u(t-Nk),...,u(t-Nk-Nb+1)) + noise'...
         ''...
         ' See IDNLARX for details.'...
         ''...
         ' 2. Hammerstein-Wiener block oriented models: These are combinations of linear'...
         ' dynamic blocks and static nonlinearties, such as'...
         ' y(t) = H(x(t)), x(t) = L(q) w(t), w(t) = G(u(t))'...
         ' where G and H are nonlinear functions and L is a linear dynamic model.'...
         ' See IDNLHW for details.'...
         ''...
         ' 3. Arbitrarily structured differential and difference state space equations.'...
         ''...
         ' xn(t)=f(x(t),u(t),par), y(t)=h(x(t),u(t),par)'...
         ''...
         ' where xn(t) is x(t+1) or d/dt x(t), and par is an unknown parameter vector.'...
         ' See IDNLGREY for details'...
         };
   case 'adv'
      DisplayStr ={...
   ...
   case 'not'
      DisplayStr = {...
         ...
         '    like Control System Toolbox. Remember this when looking at HELP. Use'...
   ...
   case 'frd'
      DisplayStr ={...
         ...
         'of a system. The object is called IDFRD (see also FRD in Control System'...
   ...
   otherwise
      ctrlMsgUtils.error('Ident:general:idhelp2',type)
