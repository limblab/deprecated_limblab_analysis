...
function [V,lamtrue,R,Re,nnobs]=gntrad(zc,par,nn,algorithm,oeflag)
...
if nu~=nz-1
    error('Ident:idobsolete:gnnew1',['Incorrect number of data columns specified. ',...
        'It should be equal to the sum of the number of inputs and the number of outputs.'])
end
...
if nargout>2, R=R1(1:n1,1:n1);Re=R1(1:n1,n1+1);end
V = V/nnobs;
...
if nr1>nRr || nr1 > nRc+1
    error('Ident:idobsolete:gnnew2','Too few data for this number of parameters.')
end
...
%sqrlam=struc.sqrlam;
sqrlam = inv(idpack.sqrtmcov(V)); % This is for the det-criterion: The gradient of
...
if oeflag
    ...
    try
        ...
        vmodel = pvset(vmodel,'A',oestab(pvget(vmodel,'A'),0.99,1));
        esqr = idpack.sqrtmcov(pvget(vmodel,'NoiseVariance'));
        [av,bv,cv,dv,kv] = idssdata(vmodel); cv1 = cv;
...
for kexp = 1:length(ze);
    ...
    for kc=1:M:Ncap
        ...
        for kl=index(:)'
            ...
            m0=parset(m0,th1);
            [A1,B1,C1,D1,K1,X1] = idssdata(m0);
            if T>0&&struc.Tflag

                [A1,B1,Cc,D1,K1] = idsample(A1,B1,C1,D1,K1,T,struc.intersample);
            end
            m0=parset(m0,th2);
            [A2,B2,C2,D2,K2,X2] = idssdata(m0);
            if T>0&&struc.Tflag
            ...
            if ~(lim==0),psitemp=psitemp.*regulder;end
        ...
        R1=R1(1:min(nRr,nRc),:);
        %    tic,   [dum,R1,E1]=qr([R1;[psi,evec]],0);
        %       R1 = R1(E1,:);toc
    end
end %kexp
if any(any(~isfinite(R1))')
    R = []; Re =[]; V = inf;
else
...
sqrlam = inv(sqrtm(V)); % This is for the det-criterion: The gradient of
% det(V) is proportional to the gradient of tr(el*inv(V)*el)
Qperp=struc.Qperp;
dkx=struc.dkx;
if back
    ...
    if ~isempty(B), % If not time series
        for kc=1:M:Ncap
            ...
            for j = 1:n*(p*(1+dkx(2))+m),   % Gradients w.r.t. A,B,C and K
                ...
                if ~(lim==0),psitmp=psitmp.*regulder(jj,:);end
