...
mode=mode(1:3);
if strcmp(mode,'sub'),sbflag=1;else sbflag=0;end
[nzp,nc]=size(zepo);
if nzp<2
...
if any(any(zsc==inf));ztemp=zsc(:);
    ztemp(ztemp==inf)=zeros(length(find(ztemp==inf)),1);zsc(:)=ztemp;end
om=2.1*pi*(1:100)/100;
w=exp(om*sqrt(-1));
...
if abs(sum(sign(inputs1)))~=nc
    disp(['WARNING: ZEPO contains a mixture of discrete and continuous models.',...
        '\nPress a key to continue, C-c to abort']),pause,
end
T=sign(inputs1(1));inputs1=abs(inputs1);zepo(1,:)=inputs1;
mo=fix(inputs1/1000);mmo=max(mo);
kko=[]; % The output indices
for k=0:mmo
    if ~isempty(find(mo==k, 1)),kko=[kko k];end
end

for ko=kko+1
    inputs=inputs1-(ko-1)*1000;
    
    kki=[]; mi=max(inputs(inputs<20));% The input indices
    mino=max(inputs(inputs>500 & inputs<520));
    if isempty(mino), kiind = 1:mi; else kiind=[1:mi 501:mino]; end
    for ki=kiind
        if ~isempty(find(inputs==ki, 1)),kki=[kki ki];end
    end
    
    %newplot;
    space=[];
    sbcount = 0;
    if strcmp(mode,'sub') && norm(kki-kki(1)*ones(1,length(kki)))>0,
        sbcount=1;
    end
    if strcmp(mode,'sam')
        mm=max(max(abs(zsc(2:nzp,(inputs)/60<1))));
        m=ceil(mm);
        %{
        if T<0,
            mar=max(max(real(zsc(2:nzp,(inputs)/60<1))));
            mir=min(min(real(zsc(2:nzp,(inputs)/60<1))));
        end
        %}
    end
    for ki=kki
        ...
        colcount=0.6;
        for k=inpc
            colcount=colcount+0.5;  
            if abs(colcount-8.1)<0.1,colcount=1.1;end
            ...
            else info=zepo(1,k:k+1)-1000*(ko-1);
                if info(1,1)>500,info=info-500;end
        ...
        if ~strcmp(mode,'sam'), hold off,end
