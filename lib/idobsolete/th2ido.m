...
poly = idpoly(ones(1,th(1,4)),num2cell(ones(1,nu)));
...
switch ps
    ...
    case 3 %% Multivariable ARX model
        ...
        m = idarx(A1,B1);
        m = pvset(m,'NoiseVariance',lam,'Ts',th(1,2));
        m = pvset(m,'CovarianceMatrix',P);
        est = pvget(m,'EstimationInfo');
    ...
    case 4 %% Discrete time user defined parameterization
        ...
        mfname=setstr(sspm);
        m = idgrey(mfname,p,'d',arg);
        m = pvset(m,'ParameterVector',p,'CovarianceMatrix',P,'NoiseVariance',lam);
        est = pvget(m,'EstimationInfo');
    case 5 %% User defined parameterization with c/d switch
        sspm=th(1,8:end);
        sspm=sspm(1:max(find(sspm~=0)));
        mfname=setstr(sspm);
        m = idgrey(mfname,p,'cd',arg);
        m = pvset(m,'CovarianceMatrix',P,'NoiseVariance',lam);
        est = pvget(m,'EstimationInfo');
end
if isa(m,'idarx')
