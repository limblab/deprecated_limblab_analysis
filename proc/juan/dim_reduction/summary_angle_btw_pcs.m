...
%       nbr_eigenvals       : nbr. of eigenvalues (dimensions) that will be
...
function [angle_mtrx, angle_lbls] = summary_angle_btw_pcs( dim_red_FR, nbr_eigenvals, labels, varargin ) 
...
% calculate angle between all possibly hyperplanes. Store angles and labels
for i = 1:(nbr_spaces-1)
    for j = (i+1):(nbr_spaces)
        angle_mtrx(i,j)     = angle_btw_pcs( dim_red_FR{i}.w, dim_red_FR{j}.w, nbr_eigenvals, show_plots );
...
title(['angle between ' num2str(nbr_eigenvals) '-D neural spaces (deg)'])
colorbar, caxis([0 90])
