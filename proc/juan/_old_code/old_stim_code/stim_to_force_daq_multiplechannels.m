...
function varargout = stim_to_force_daq_multiplechannels( varargin )
...
nbr_ch_found                = 0;
for i = 1:length(s_f_params.stim_elecs)
    nbr_ch_found            = nbr_ch_found + numel(find( hw.gv.stim_ch == s_f_params.stim_elecs(i)));
end

% quit if the specified channels (in 'stim_trig_avg_params.stim_elecs') do
% not exist
if isempty(hw.gv.stim_ch)
    cbmex('close');
    error('ERROR: no stimulator found!');
elseif ( nbr_ch_found < length(s_f_params.stim_elecs) ) % ToDo: double check syntaxis 
    cbmex('close');
...
%                                 'PL = 1,0,;'];
    ...
    fs_string               = ['FS = '];
    pl_string               = ['PL = '];
    for ii = 1:numel(s_f_params.stim_elecs)
       
        elec_string         = [elec_string  num2str(s_f_params.stim_elecs(ii))      ','];
        tl_string           = [tl_string    num2str(s_f_params.train_duration)      ','];
        freq_string         = [freq_string  num2str(s_f_params.stim_freq)           ','];
        dur_string          = [dur_string   num2str(s_f_params.stim_pw)             ','];
        amp_string          = [amp_string   num2str(round(hw.gv.stim_amps(i)/s_f_params.stimulator_resolut)) ','];
        td_string           = [td_string    num2str(s_f_params.pre_stim_win/1000)   ','];
        fs_string           = [fs_string    '0,'];
        if mod(ii,2) == 0
            pl_string       = [pl_string '0,'];
        elseif mod(ii,2) == 1
            pl_string       = [pl_string '1,'];
        end
    end
    elec_string             = [elec_string  num2str(s_f_params.sync_out_elec)           ',; '];
    tl_string               = [tl_string    num2str(ceil(1/s_f_params.stim_freq*1000))  ',; '];
    freq_string             = [freq_string  num2str(s_f_params.stim_freq)               ',; '];
    dur_string              = [dur_string   num2str(s_f_params.stim_pw)                 ',; '];
    amp_string              = [amp_string   num2str(3/s_f_params.stimulator_resolut)    ',; '];
    td_string               = [td_string    num2str(s_f_params.pre_stim_win/1000)       ',; '];
    fs_string               = [fs_string    '0,; '];
    pl_string               = [pl_string    '1,;'];

    stim_string             = [elec_string, tl_string, freq_string, dur_string, amp_string, td_string, fs_string, pl_string] ;
...
if s_f_params.save_data_yn
    ...
    disp(' ');
    save(hw.matlab_full_file_name,'force','s_f_params');
