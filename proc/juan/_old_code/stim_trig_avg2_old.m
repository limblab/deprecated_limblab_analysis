...
% quit if the specified channel (in 'sta_params.stim_elec') does
% not exist, or if the sync_out channel does not exist 
if ~find( hw.gv.stim_ch == sta_params.stim_elec) 
    cbmex('close');
    error('ERROR: stim channel not found!');
end

% nbr_ch_found                = 0;
% for i = 1:length(sta_params.stim_elec)
%     nbr_ch_found            = nbr_ch_found + numel(find( hw.gv.stim_ch == sta_params.stim_elec));
% end
...
% the loop that stimulates and records EMGs
...
%             end
    ...
    % ----- This part was to fix a bug in the old cbmex code
    ...
    if numel(cell2mat(ts_cell_array(hw.cb.stim_trig_ch_nbr,2))) ~= hw.cb.nbr_stims_this_epoch
        ...
        disp(' ');
        disp(['all sync pulses succesfully detected when stimulating ch. #' num2str(sta_params.stim_elec)]);
    end
...
%         stem(ts_sync_pulses_its_freq,ones(length(ts_sync_pulses),1)*-5000,'marker','none','color','r'), legend('analog signal','time stamps')
    ...
    % ----- End of the part was to fix a bug in the old cbmex code

    
    % this is a temporal fix to ignore the data when the analog and ts are not synhronized
    if abs( ts_first_sync_pulse_analog_signal - ts_sync_pulses_its_freq(1) ) <  emg.fs/1000
...
%                 ts_sync_pulses(end) = [];
        ...
        for iii = 1:min(length(ts_sync_pulses),length(emg.evoked_emg))
            ...
            if (trig_time_in_emg_sample_nbr + (sta_params.t_after + sta_params.t_before)*emg.fs/1000 ) > length(emg.data)
                ...
                emg.evoked_emg(:,:,iii+hw.cb.ind_ev_emg)    = emg.data( trig_time_in_emg_sample_nbr : ...
                    (trig_time_in_emg_sample_nbr + emg_win_length_in_samples), : );
            end
        end
