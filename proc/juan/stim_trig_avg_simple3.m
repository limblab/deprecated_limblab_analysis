...
%
%       varargout = stim_trig_avg_simple( varargin )
%
%       Input parameters: 1) 'sta_params'; otherwise read from
%       sta_params_default
%       Output parameters: 1) 'emg', 2) 'sta_params'


% %%%%%%%%%%%%
%   Some known issues:
%       - The EMG sampling freq is hardcoded, to 2 kHz
%       - The resistor used in the analog input is hard-coded


function varargout = stim_trig_avg_simple3( varargin )
...
% 'emg.evoked_emg' has dimensions EMG signal -by- EMG channel- by- stimulus nbr -by- cortical electrode nbr
...
% check that the stimulation amplitude is not too large ( > 90 uA or > 1 ms)
...
for i = 1:length(sta_params.stim_elecs)
    ...
    for ii = 1:hw.cb.nbr_epochs
        ...
        for iii= 1:emg.nbr_emgs
            emg.data(:,iii,i)   = double(aux{iii,1}); % ToDo: improve - esto esta dando problemas
        end
...
%                 ts_sync_pulses(end) = [];
            elseif (length(emg.data) - sta_params.t_after) < floor(ts_sync_pulses(end)/30000*emg.fs) % ToDo: improve; the -10 is because sometimes it crashed because the code is different to line 324
                ts_sync_pulses(end) = [];
            end
    ...
    %----------------------------------------------------------------------
    % Perform the STA and plot the data


    % filter if specified in sta_params.fc_hp_filt
    if sta_params.fc_hp_filt ~= 0
        for ii = 1:emg.nbr_emgs

            emg.STA(:,ii)       = mean( abs(filtfilt( emg.online_filt.b, emg.online_filt.a, squeeze(emg.evoked_emg(:,ii,:,i)))),2);
            emg.STA_std(:,ii)   = std( abs(filtfilt( emg.online_filt.b, emg.online_filt.a, squeeze(emg.evoked_emg(:,ii,:,i)))),0,2);
        end
    else
        for ii = 1:emg.nbr_emgs

            emg.STA(:,ii)       = mean(abs(squeeze(emg.evoked_emg(:,ii,:,i))),2);
            emg.STA_std(:,ii)   = std(abs(squeeze(emg.evoked_emg(:,ii,:,i))),0,2);
        end
    end
    ...
    if ishandle(sta_fig.h)
        ...
        % code for 11 EMGs
        for ii = 1:emg.nbr_emgs
           subplot(3,4,ii), hold on, plot(sta_fig.t,emg.STA(:,ii),'color',sta_fig.colors_plot(i,:))
%            plot(sta_fig.t,emg.STA(:,ii)-emg.STA_std(:,ii),'color',sta_fig.colors_plot(i,:),'linestyle','-.')
%            plot(sta_fig.t,emg.STA(:,ii)+emg.STA_std(:,ii),'color',sta_fig.colors_plot(i,:),'linestyle','-.')
           ylabel(emg.labels(ii)),xlim([sta_fig.t(1) sta_fig.t(end)])
        end
    end
    
    
    % delete the variables we don't need
    clear analog_data ts_cell_array aux ts_sync_pulses;
end
