...
%
% Notes: 
%   - The BDF is cropped between the first and last position reads, to
%   avoid rounding problems and ensure compatibility with
%   convertBDF2binned.m
...
if nargout == 2
    ...
    bin_pars.starttime  = ceil(bdf(1).pos(1,1)/bin_size)*bin_size;
    bin_pars.stoptime   = floor(bdf(1).pos(end,1)/bin_size)*bin_size;
...
% otherwise, just bin the spikes
    ...
    t_start             = ceil(bdf(1).pos(1,1)/bin_size)*bin_size;
    t_end               = floor(bdf(1).pos(end,1)/bin_size)*bin_size;
