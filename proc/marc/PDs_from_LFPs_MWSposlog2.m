function [LFPfilesPDs,bootstrapPDS,LFP_counts]=PDs_from_LFPs_MWSposlog2(root,LFPs,fbandst,fbanden,startword,rewardword,shiftstart,lag,bin_size,pvallim)
...
for k=1:length(fbandst)
    ...
    for x = 1:length(LFPs)
        ...
        bootstrapPDS{x,k} = bootstrap(@vector_sum_PDs_LFPs2,LFP_counts{x,k}, 'all', 1000);
