function [LFPfilesPDs]=PDs_from_LFPs_MWS(root,LFPs,fbandst,fbanden,startword,rewardword,shiftstart,lag,bin_size,pvallim)
...
%-duration of a trial.
%lag time difference (in seconds) between the handle velocity and t'he spike cout periods. for M1 the lag should  be negative as spike preceed the movement whereas for s1 lag should be positive
...
%%%in freq band
degres=45;%if you change degres (degrees) here change it also in vector_sum_PDs_LFPs
...
%     starts=starts+abs(shiftstart);
    ...
    while ends(end)>data.vel(end,1)-bin_size
    ...
    for i=1: length(starts)
        %% x and y velocities for each cues in each trial
        rt=starts(i);

 
        while rt<ends(i)+bin_size+lag
            RTind=floor(rt*LFPrate);
            xvel=data.vel(find(data.vel(:,1)>=rt,1),2);
            xvel2=data.vel(find(data.vel(:,1)>=rt+bin_size,1),2);
            
            yvel=data.vel(find(data.vel(:,1)>=rt,1),3);
            yvel2=data.vel(find(data.vel(:,1)>=rt+bin_size,1),3);
            %% vector calculate
            xvelvect=xvel2-xvel;
            yvelvect=yvel2-yvel;
            yud=yud+1;
            direction(yud)= atan2(yvelvect,xvelvect);
...
%                 lfp=zeros(1,length(LFPsample));
              ...
              fst=find(fsam>=fbandst(k),1);fend=find(fsam>=fbanden(k),1); 
