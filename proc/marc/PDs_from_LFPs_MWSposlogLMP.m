function [LFPfilesPDs,bootstrapPDS,LMPcounts]=PDs_from_LFPs_MWSposlogLMP(root,LFPs,fbandst,fbanden,startword,rewardword,shiftstart,lag,bin_size,pvallim)
...
%%%in freq band
%vpos uses position, and also uses the MEAN over the whole band, not the
%MAX, as in the prior version
%vposlog takes log power
%vposlogLMP calculates LMP only
...
for i=1: length(starts)
    ...
    rt=starts(i);
%     LMP=zeros((ends(i)+bin_size+lag)/bin_size,length(LFPs));
    ...
    while rt<ends(i)+bin_size+lag
        ...
        for j=1:length(LFPs)
            ...
            LMP(yud,chi)=mean(LFP(RTind+lag*LFPrate:RTind+bin_size*LFPrate+lag*LFPrate));
...
%                 fend=find(fsam<=fbanden(k),1,'last');
%                 
%                 LFPall{1,k}(yud,chi)=10*mean(log10(gat(fst:fend)));        %use mean power over all freqs instead of max
%             end
        end
...
%         chLFP=LFPall{1,k}(:,r);
        ...
        for i=1:length(os)-1
            LMPcounts{r}{i}=chLMP(os(i) < direction & direction < os(i+1));
...
%     disp(['band',int2str(k),'/',int2str(length(fbandst))]);toc
    for x = 1:length(LFPs)
        ...
        bootstrapPDS{x} = bootstrap(@vector_sum_PDs_LFPs2,LMPcounts{x}, 'all', 1000);
        ss=bootstrapPDS{x}(:,1);
...
% for k=1:length(fbandst)
    ...
    for x = 1:length(LFPs)
        ...
        PDMatrix(x,5)= LFPs(x);
        PDMatrix(x,4) =mean(bootstrapPDS{x}(:,2));
        PDMatrix(x,6:7)= prctile(bootstrapPDS{x}(:,2),[lowCI upCI]); %bootstrap stats of magnitude
    end;
% end
...
LFPfilesPDs{4,1}='This is LMP PD data';
