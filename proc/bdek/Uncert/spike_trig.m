...
for i = 1:length(comp_tt)
    ...
    trial_spikes = [trial_spikes ; spikes(spikes > time(1) & spikes < time(2))];
...
for i = 1:length(trial_spikes)
    ...
    if i > 2
    if (norm_v(i-1,3) == 3.5 && norm_v(i,3) == 0.5 && markfirst == 0)
        
        legend('High Variance Feedback','Low Variance Feedback');
        markfirst = 1;
    end
    end
...
title('Spiking Behavior','FontSize',18);
