...
if ~iscellstr(filenames)
    load(filenames,'bdf','trial_table','table_columns')
else
    trial_table_concat = [];
    for ifile = 1:length(filenames)
        load([filenames{iFile}.datapath 'Processed\' filenames{ifile}],'bdf','trial_table','table_columns')
        bdf_all(ifile) = bdf;
        ...
        if ifile>1
            trial_table(:,[table_columns.cursor_on_ct table_columns.start...
                ...
                trial_table(:,[table_columns.cursor_on_ct table_columns.start...
                table_columns.bump_time table_columns.end]) + end_time(ifile-1);
            bdf_concat.pos = [bdf_concat.pos ; [bdf.pos(:,1)+end_time(ifile-1) bdf.pos(:,2:3)]];
            bdf_concat.vel = [bdf_concat.vel ; [bdf.vel(:,1)+end_time(ifile-1) bdf.vel(:,2:3)]];
        else
    ...
    for i=1:length(bdf.units)
        ...
        for ifile = 2:length(filenames)
            bdf_concat.units(i).ts = [bdf_concat.units(i).ts; bdf_all(ifile).units(i).ts + end_time(ifile-1)];
        end
    end
