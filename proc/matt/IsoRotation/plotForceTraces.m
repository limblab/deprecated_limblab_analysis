function forceOut = plotForceTraces(trialTable, force, holdTime)
...
endTimes = trialTable(:,8)-holdTime;
...
for i = 1:length(targets)
    ...
    for ind = 1:length(relInds)
        ...
        tempForce = force(force(:,1) >= trialTable(relInds(ind),interval) & force(:,1) <= trialTable(relInds(ind),8),:);
    ...
    meanForce = [mean(allForceX,1); mean(allForceY,1)];
    stdForce = [std(allForceX,1); std(allForceY,1)];
...
plotMeanByTarget(meanIso, stdIso);
