...
%   sg: spike guide
function tuningOverMovement2(filename,doPlots)
close all
...
[pdFull, pFull] = computeFullTuning(neural, offsetFR, thetaTarg, trialTable, tau,false);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Hold Period tuning
[pdHold, pHold] = computeHoldPeriodTuning(neural, offsetFR, thetaTarg, trialTable, holdTime, tau,false);
...
tcsT = zeros(length(neural),3,size(timeCenters,2));
tcsM = zeros(length(neural),3,size(timeCenters,2));
%%%
...
%     ints = [trialTable(:,7), trialTable(:,8)];
    ...
    fr = calculateFR(neural,ints,timeCenters,winSize,iCenter);
