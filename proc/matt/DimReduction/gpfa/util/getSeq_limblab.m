...
for n = 1:length(dat)
    ...
    T    = floor(dataBinWidth * size(dat(n).spikes, 2) / binWidth);
    ...
    for t = 1:T
        ...
        % they used to assume dataBinWidth is 1, so you can jump by
        % binWidth samples each bin. However, we now allow for a flexible
        % dataBinWidth so you jump by binWidth/dataBinWidth samples.
        iStart = round(binWidth/dataBinWidth) * (t-1) + 1;
        iEnd   = round(binWidth/dataBinWidth) * t;
