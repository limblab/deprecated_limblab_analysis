function plotEachDimVsTime_Matt(seq, xspec, binWidth, varargin)
%
...
f = gcf;
useColor = 0.2*[1 1 1]; % gray for default
nPlotMax  = 20;
redTrials = [];
blueTrials = [];
nCols     = 4;
...
if colorGradient
    useColor = [linspace(0,1,min(length(seq), nPlotMax))', zeros(min(length(seq), nPlotMax),1),linspace(1,0,min(length(seq), nPlotMax))'];
end
