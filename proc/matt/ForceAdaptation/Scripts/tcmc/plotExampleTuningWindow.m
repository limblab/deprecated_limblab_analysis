...
% useDate = '2013-09-03';
% data = load(fullfile(baseDir, useDate,['CO_VR_BL_' useDate '.mat']));
...
usePos = pos(t >= trial(4) & t<trial(4)+winSize,:);
...
usePos = pos(t >= trial(4) & t<trial(4)+winSize,:);
% plot([usePos(1,1) usePos(end,1)]-xoffset,[usePos(1,2) usePos(end,2)]-yoffset,'k--','LineWidth',2);
arrow('Start',[usePos(1,1)-xoffset usePos(1,2)-yoffset],'Stop',[xcenter ycenter],'Width',3);
plot([usePos(1,1) usePos(end,1)]-xoffset,[usePos(1,2) usePos(1,2)]-yoffset,'k--','LineWidth',2);
...
% add theta marker
usePos = pos(find(t>=tpeak,1,'first'),:);
text(usePos(1)-xoffset-0.2,0.9,'\theta','FontSize',24);
