function [R2, varargout] = mfxval_Matt(binnedData, options,trialStarts,trialEnds)
...
for i=0:nfold-1
    ...
    testDataStart = trialStarts(i+1);      %move the test block from beginning of file up to the end
    testDataEnd = trialEnds(i+1);
    
    [trainData,testData] = splitBinnedData_Matt(binnedData,testDataStart,testDataEnd);
    
    % make sure there is actually test data
    errorFlag = 0;
    if numel(testData.timeframe) > 1
        ...
        if i == 0
            ...
            AllPredData.trialStarts = PredData.timeframe(1);
            AllPredData.trialEnds = PredData.timeframe(end);
            AllPredData.trueTrial = i+1;
        else
            ...
            % save timestamp of when this trial starts
            AllPredData.trialStarts = [AllPredData.trialStarts; PredData.timeframe(1)];
            AllPredData.trialEnds = [AllPredData.trialEnds; PredData.timeframe(end)];
            AllPredData.trueTrial = [AllPredData.trueTrial; i+1];
        end
...
if options.plotflag
    ActualvsOLPred_Matt(binnedData,AllPredData,options.plotflag);
end
