function build_KF_Matt(dataFile,delayBins,startTime,endTime,genPlots)
...
if ~isstruct(dataFile) % assume you are receiving a filename
...
elseif isfield(dataFile,'units') % assume you are receiving a BDF
    out_struct = dataFile;
    clear dataFile;
    name = out_struct.meta.filename;
    ext = '.mat';
...
if exist('out_struct','var')
    ...
    binnedData = convertBDF2binned_Matt('temp_out_struct', 0.05, startTime, endTime);
...
elseif isfield(dataFile,'timeframe') %assume you are receiving a binnedData file
    binnedData = dataFile;
    clear dataFile;
    name = binnedData.meta.filename;
    ext = '.mat';
else
...
% PCcoeffs = pca(binnedData.spikeratedata);
