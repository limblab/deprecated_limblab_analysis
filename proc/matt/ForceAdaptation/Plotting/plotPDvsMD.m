function plotPDvsMD(bl,ad,wo,classes,tracking,sigMethod,saveFilePath)
        ...
        for unit = 1:size(sg_master,1)
            ...
            if ismember(sg_master(unit,:),tuned_cells,'rows')
                ...
                if ~any(diff(useComp(relCompInd,:)))
                    ...
                    plot(mds(1:2,1)',pds(1:2,1)'.*180/pi,'Color',useColor,'LineWidth',1);
                    
                    plot(mds(1,1),pds(1,1).*180/pi,'+','Color',useColor,'LineWidth',3);
                    plot(mds(2,1),pds(2,1).*180/pi,'d','Color',useColor,'LineWidth',3);
%                     plot(mds(3,1),pds(3,1).*180/pi,'d','Color',useColor,'LineWidth',3);
                    
                    rx = (mds(1,3)-mds(1,2))/2;
                    ry = (pds(1,3)-pds(1,2))*90/pi;
                    ellipse(mds(1,2)+rx,pds(1,2).*180/pi+ry,rx,ry,useColor)
                    
                    rx = (mds(2,3)-mds(2,2))/2;
                    ry = (pds(2,3)-pds(2,2))*90/pi;
                    ellipse(mds(2,2)+rx,pds(2,2).*180/pi+ry,rx,ry,useColor)
