function [tunCurves,confBounds,rs,boot_pds,boot_mds,boot_bos] = regressTuningCurves_Moran(fr,theta,vel,sigTest,varargin)
% Compute cosine tuning curves relating neural activity to output using a
% linear regression method to get preferred directions. This version is
% based on the Moran/Schwartz 1999 model that incorporates movement speed.
% Uses model:
%   
%       fr - b3 = V * (b0 + b1*cos(theta + b2)
%
...
%   sigTest: Cell array of parameters for testing significance.s
%     OPTIONS:
...
%
%%%%%%
% written by Matt Perich; last updated December 2014
%%%%%

disp('%%%%%%%%%%%%%%%%%%');
disp('DOING MORAN-ESQUE SPEED MODEL');
disp('&&&&&&&&&&&&&&&&&&');
...
switch lower(sigTest{1})
    case 'bootstrap'
        ...
        for iter = 1:numIters
            ...
            if doMeanFR
                ...
                mtempVel = zeros(length(udir),size(tempfr,2));
                for idir = 1:length(udir)
                    for unit = 1:size(fr,2)
                        mtempfr(idir,unit) = mean(tempfr(tempTheta(:,unit)==udir(idir),unit));
                        mtempVel(idir,unit) = mean(tempVel(tempTheta(:,unit)==udir(idir),unit));
                    end
                end
            ...
            [tunCurves,r] = regressTCs(tempfr,tempTheta,tempVel,doPlots);
    ...
    case 'anova'
        ...
        md_sig = [NaN,NaN];
        bo_sig = [NaN,NaN];
        [tunCurves,rs] = regressTCs(fr,theta,vel,doPlots);
...
function [tunCurves,r] = regressTCs(fr,theta,vel,doPlots)
...
for iN = 1:size(fr,2)
    ...
    ct = cos(theta(:,iN));
    v = vel(:,iN);
    X = [ones(size(theta(:,iN))) v.*st v.*ct v];
