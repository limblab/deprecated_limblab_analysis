...
% Build Decoder
[filter OffLPredData, trainDataSet R2] = BuildFilterWiener(1:9,2);
R2_mean = mean(R2);
...
for i = 1:numEMGs
    figure; plot(xa,R2AF_r(:,i)); ylim([0 1]);
...
legend('EMG templates','Real EMGs','Location','Best')
title(sprintf('Average Accross Muscles\nAdaptive Decoder'));
xlabel('Time (min)');
ylabel('R2');    
[mean(R2AF_e(:));mean(R2AF_r(:))]
