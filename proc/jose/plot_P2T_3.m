...
%         ylabel('trials'); 
        ...
        figure(9)   
        for k=1:101
            plot_ellipse(std_path(1,k),std_path(2,k),m_paths_x(i,k),m_paths_y(i,k),0,rgb_c(tgts(i),:)); 
            hold on;
        end
    end
    for i=1:length(tgts)
        figure(9)
        plot(m_paths_x(i,:),m_paths_y(i,:),color_l(tgts(i),:),'LineWidth',2,'MarkerEdgeColor',...
        colors(tgts(i)),'MarkerFaceColor',colors(tgts(i)),'MarkerSize',1);hold on
        plot(m_paths_x(i,1),m_paths_y(i,1),color_s_e(tgts(i),:),'LineWidth',2,'MarkerSize',7)
        plot(m_paths_x(i,end),m_paths_y(i,end),color_s_e(tgts(i),:),'LineWidth',2,'MarkerSize',7)
        axis([-12 12 -12 12]);
        axis equal;
