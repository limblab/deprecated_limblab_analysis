function [spm,spme] = adapt_plot_stats(all_stats)

[num_conditions,num_tests] = size(all_stats);
num_targets = 8;

%success per minute
spm  = nan(num_targets+1,num_conditions,num_tests);
spme = spm;
t2t  = spm;
t2te = spm;
pl   = spm;
        ...
        if ~isempty(all_stats{cond,day})
            ...
            for min = 1:num_mins
                for tgt = 1:num_targets
                    SR(tgt,min) = all_stats{cond,file}.succ_per_min{tgt}(min);
                end
            end
            ...
            t2t(end,cond,day) = mean(t2t(~isnan(t2t(1:end-1,cond,day)),cond,day));
