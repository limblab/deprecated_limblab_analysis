...
hold on;
numpts = max([size(MR2a,1) size(MR2f,1)]);

%calculate SD around mean curve
x = round([1:numpts numpts:-1:1])*res.info.foldlength/60;
...
area(x,yf,'FaceColor',gray,'LineStyle','none');
% area(x,yff,'FaceColor',light_blue,'LineStyle','none');
area(x,ya,'FaceColor',pink,'LineStyle','none');
% area(x,yaf,'FaceColor',light_green,'LineStyle','none');

x = (1:numpts)*res.info.foldlength/60;
hf = plot(x,Mf,'k.-','MarkerSize',12,'LineWidth',2);
ha = plot(x,Ma,'r.-','MarkerSize',12,'LineWidth',2);
% hff = plot(1:numpts,Mff,'k.-');
% haf = plot(1:numpts,Maf,'g.-');
legend([hf,ha],'fixed decoder', 'adaptive decoder');
% legend([hf,ha,hff,haf],'fixed model','adaptive model','fixed no drop/perm','adapt, no drop/perm');
title('Mean R^2 accross muscles');
