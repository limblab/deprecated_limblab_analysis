function [ts] = Get_TPR_ts_MG(out_struct)
    ...
    %%---------------------------------%
    % get matching Touch_Pad and Reward ts
    %%---------------------------------%
    
    TPR_pairs = Get_Words_ts_pairs(w.Start, w.Touch_Pad, w.Reward, out_struct.words);
    
    numRewards = size(TPR_pairs,1);
    
    TP_ts = [TPR_pairs(:,1) zeros(numRewards,2)];
    Reward_ts = [TPR_pairs(:,2) zeros(numRewards,2)];
