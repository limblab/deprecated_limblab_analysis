function [filter, varargout]=BuildModel_Templates(binnedData, dataPath, fillen, UseAllInputsOption, PolynomialOrder, varargin)
...
%       varargin = {PredEMG, PredForce, PredCursPos,Use_Thresh} : flags to include EMG, Force, Cursor Position and Thresholding in the prediction model (0=no,1=yes)
    ...
    if ~(PredEMG || PredForce || PredCursPos)
        disp('No Outputs are Selected, Model Building Canceled');
        return;
    end
