...
numlags  = 10; numsides = 1; fs =1;

H = zeros((numNeur-1)*numlags,numNeur);
...
for n = 1:numNeur
    ...
    %train decoder:
    X = trainData.spikeratedata(:,otherNs);
    Y = trainData.spikeratedata(:,n);
    H(:,n) = filMIMO3(X,Y,numlags,numsides,fs);
    
    %use decoder to predict neuron
    X = testData.spikeratedata(:,otherNs);
    Yact = testData.spikeratedata(:,n);
    [Y,Xnew,Yact] = predMIMO3(X,H(:,n),numsides,fs,Yact);
...
%     [YHat,YLo,YHi] = glmval(B,NXSCtest,'log',Stats);
    ...
    for i= 1:numSteps
        start = 1+(i-1)*corrWindow;
        stop  = min([start+corrWindow length(Y)]);
        CheckTimes(i,1) = stop;
        Results(i,n) = CalculateR2(Y(start:stop),Yact(start:stop));
