function [vaf_all1feat,vmean,vsd,y_test,y_pred,varargout] = predictionsfromfp6_all1featDecoders(sig, signal, numfp, binsize, folds,numlags,numsides,samprate,fp,fptimes,analog_times,fnam,varargin)
...
% loop over the single features.
for featShift=0:(length(r1)-1)
    [bestf,bestc]=ind2sub(size(squeeze(PB(:,:,1))),featind((1:nfeat)+featShift));
...
%     tic
    ...
    fprintf(1,'featShift=%d\n',featShift)
    vaf_all1feat{featShift+1}=vaf;
    bestc_all1feat{featShift+1}=bestc;
    bestf_all1feat{featShift+1}=bestf;
end
...
if nargout>5
    ...
    if nargout>7
        ...
        if nargout>8
            varargout{4}=vaftr;
            if nargout>9
                varargout{5}=bestf_all1feat;
                varargout{6}=bestc_all1feat;
                if nargout>11
                    varargout{7}=H;
                end
                if nargout>12
