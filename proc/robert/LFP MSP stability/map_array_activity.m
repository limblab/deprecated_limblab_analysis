...
mapvect=map_matrix(:);
ind=zeros(1,100);
Array_Activity_Map_V=zeros(1,100);
    for j=1:100
        if mapvect(j)
            Array_Activity_Map_V(j)= Data(spike_channel_numbers==mapvect(j));
        else
            Array_Activity_Map_V(j)=0;
%         ind(i,j)= j;
        end
    end
% % ind = nonzeros(ind);    %Get rid of the zero elecments
% 
% 
% for j=1:100
%  Array_Activity_Map_V(j)= Data(ind(j));
% end
 Array_Activity_Map=reshape(Array_Activity_Map_V,10,10);
% end
