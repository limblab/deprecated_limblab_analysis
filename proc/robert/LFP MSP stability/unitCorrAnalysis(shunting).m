...
for n=1:length(allChewieNames)
    try
        ...
        binnedData = convertBDF2binned(out_struct,struct('binsize',binsize, ...
            'starttime',starttime,'stoptime',stoptime,'EMG_hp',5,'EMG_lp',0, ...
            'minFiringRate',MinFiringRate)); disp('done')
...
%% to use pre-existing analysis
load('E:\personnel\RobertF\monkey_analyzed\LFP MSP stability\allUnitsXcorr(shunt_analysis).mat')
...
if ~exist('ind','var')
    ind=1:size(corrMatAll_Chewie,3);
end
[r,c]=find(triu(mean(corrMatAll_Chewie(:,:,ind),3)>0.3,1)); clear ind
Chewie_shuntedCh=unique([r; c]); clear r c
%%
...
if ~exist('ind','var')
    ind=1:size(corrMatAll_Mini,3);
end
[r,c]=find(triu(mean(corrMatAll_Mini(:,:,ind),3)>0.3,1)); clear ind
Mini_shuntedCh=unique([r; c]); clear r c
