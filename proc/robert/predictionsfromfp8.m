function [vaf,vmean,vsd,y_test,y_pred,varargout] = predictionsfromfp8(sig, signal, numfp, binsize, folds,numlags,numsides,samprate,fp,fptimes,analog_times,fnam,varargin)
...
else % stay inside the boundaries of the non-validation data
    for i = 1:(folds-1)
        ...
        x_train = [x(1:fold_start,:); x(fold_end:(validationFoldStart-1),:)];
        y_train = [y(1:fold_start,:); y(fold_end:(validationFoldStart-1),:)];
