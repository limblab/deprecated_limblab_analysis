function [vaf,vmean,vsd,y_test,y_pred,varargout] = predictionsfromfp5allMOD(sig, signal, numfp, ...
...
%     tmp=detrend(tmp);
    tmp=win.*tmp;
    tfmat(:,:,i)=fft(tmp(1:end-1,:),wsz);      %tfmat is freqs X chans X bins
    %     =tftmp(2:(wsz/2+1),:);
    clear tmp
end
clear fpf
freqs=linspace(0,1,wsz/2+1)*samprate/2;
% the precision of freqs is determined by the window size wsz.  At wsz=256,
% the first nonzero element of freqs is 7.38, which skips entirely the
% delta band.  So, window at wsz=512 or greater.  Small window lengths may
% be why we're never seeing anything in the lower frequency bands in terms
% of bestf
...
tic
%% Calculate bandpower
Pmat=tfmat(1:length(freqs),:,:).*conj(tfmat(1:length(freqs),:,:))*0.75;   
...
%     x_test{i} = zscore(x_test{i});
    ...
    if exist('PolynomialOrder','var') && PolynomialOrder~=0
