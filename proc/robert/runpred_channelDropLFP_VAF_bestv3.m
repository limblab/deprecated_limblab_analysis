function runpred_channelDropLFP_VAF_bestv3(PathName)
...
for i=skipAhead:length(MATfiles)
    ...
    EMGVAFfoldsLFP=cell(length(numfp),1);
...
%         fpUse(bestcU(1:n),:)=[];
        numfp=size(fpUse,1);
        disp(['LFP: ',num2str(numfp),' channels'])
        if numfp*6 < 150
            nfeat=numfp*6;
        else
            nfeat=150;
        ...
        vmean
        EMGVAFfoldsLFP{numfp}=vaf;
    end
    % fpUse will now have the same size as bestcU.
    fpRemainder=fp(setdiff(1:size(fp,1),bestcU),:);
    for n=1:size(fpRemainder,1)
        numfp=numfp+1;
        % arbitrarily, add remaining elements in whatever order they first occured in fp
        fpUse(numfp,:)=fpRemainder(n,:);  
        if numfp*6 < 150
            nfeat=numfp*6;
        else
            nfeat=150;
    ...
    save(fullfile(PathName,folderNew,[fnam,' chan_drop.mat']), ...
        'EMGVAFmallLFP','EMGVAFsdallLFP','EMGchanNames','EMGVAFfoldsLFP')
end