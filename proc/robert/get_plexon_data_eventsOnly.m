function out_struct = get_plexon_data_eventsOnly(varargin)
...
% $Id: get_plexon_data.m 308 2010-10-22 17:10:15Z brian $
...
%% Data From PLX File    
    ...
    out_struct.meta = struct('filename', OpenedFileName, 'datetime', ...
        ...
        'bdf_info', '$Id: get_plexon_data.m 308 2010-10-22 17:10:15Z brian $');

    try
        [n, strobe_ts, strobe_value] = plx_event_ts(filename, 257);
    catch err
        if isequal(err.identifier,'MATLAB:unassignedOutputs')
            disp('exiting...')
            return
        end
    end
...
%     out_struct.units = get_units_plx(filename, opts);
    out_struct.raw = get_raw_plx_eventsOnly(filename, opts);    
...
%% Extract data from the raw struct
    ...
    out_struct = calc_from_raw_eventsOnly(out_struct,opts);
