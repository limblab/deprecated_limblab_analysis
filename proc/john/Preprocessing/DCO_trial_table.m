...
remove_databurst = [];
bdf.words = bdf.words(1:find(bdf.words(:,1)<bdf.pos(end,1),1,'last'),:);  % Remove last partial second
bdf.databursts = bdf.databursts(1:find([bdf.databursts{:,1}]<bdf.pos(end,1),1,'last'),:);
...
% Remove all NaNs and only preserve 4th (GO CUE) column
% trial_table = trial_table(isfinite(trial_table(:,4)));
