function [] = spontFRroutine(bdf,tt,stimCycle)
bdfname = inputname(1);
...
% for iNeuron = 1:length(sorted_units)
for iNeuron = 5
    oneNeuronSpontFRS(iNeuron);
    pause
end

function [] = oneNeuronSpontFRS(n_neuron)
    ...
    if 1
        ...
        for iBlock = 1:n_blocks
            ...
            errUp = yVal + stdDev; errDown = yVal - stdDev;
            
                % Plot iBlock's mean FR and std. deviation
            line('XData',[min(X) max(X)], 'YData',[yVal yVal],'LineStyle', '-','Color','k')
            line('XData',[min(X) max(X)], 'YData',[errUp errUp],'LineStyle', '-','Color','m')
            line('XData',[min(X) max(X)], 'YData',[errDown errDown],'LineStyle', '-','Color','m')
            Y = [repmat(errUp,1,length(X)) fliplr(repmat(errDown,1,length(X)))];
            X = [X fliplr(X)];
            patch(X,Y,color,'FaceAlpha',0.25,'EdgeAlpha',0);            
            ylim([errDown*1.2 max(spontRatePerTrial)*1.2])
            set(gca,'XTick',[0:n_blocks]);
        end
    end
