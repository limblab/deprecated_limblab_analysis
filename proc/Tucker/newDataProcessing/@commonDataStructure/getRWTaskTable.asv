function getRWTaskTable(cds,times)
    ...
    wordGo = hex2dec('30');
    goCues =  bdf.words(bitand(hex2dec('f0'), bdf.words(:,2)) == wordGo, 1);
    goCodes=  bdf.words(bitand(hex2dec('f0'), bdf.words(:,2)) == wordGo, 2)-wordGo;
    ...
    switch db_version
        case 0
            ...
            if datenum(bdf.meta.datetime) < datenum('3/9/2009') 
                numTgt = (bdf.databursts{1,2}(1)-18)/8;
            else
        ...
        case 1
            ...
            for trial = 1:numel(cds.databursts.ts)
                if (bdf.databursts{trial,2}(1)-18)/8 ~= numTgt
                ...
                % Go cues
                idxGo = find(goCues > start_time & goCues < stop_time);
                ...
                %find target centers
                ctr=[bytes2float(cds.databursts.db(trial,15:18))    ,   bytes2float(cds.databursts.db(trial,hdr_size+1:end))];
                % Offsets, target size
                xOffset = bytes2float(bdf.databursts{trial,2}(7:10));
                yOffset = bytes2float(bdf.databursts{trial,2}(11:14));
                tgtSize = bytes2float(bdf.databursts{trial,2}(15:18));

                % Build arrays
                goCueList(trial,:)=         goCue;              % time stamps of go_cue(s)
                goCodeList(trial,:)=        goCode;             % ?
                numTgts(trial,:)=           numTgt;             % max number of targets
                numAttempted(trial,:)=      tgtsAttepmted;      % ?
