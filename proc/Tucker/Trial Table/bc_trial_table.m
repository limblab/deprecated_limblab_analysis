...
%  */
        ...
        staircase_id=db(10);
        target_angle=bytes2float(db(15:18));
        bump_dir=bytes2float(db(19:22));
        random_tgt_flag=db(23);
        tgt_dir_floor=bytes2float(db(24:27));
        tgt_dir_ceil=bytes2float(db(28:31));
        bump_mag=bytes2float(db(32:35));
        bump_dur=bytes2float(db(36:39));
        bump_ramp=bytes2float(db(40:43));
        rand_bump_flag=db(44);
        bump_floor=bytes2float(db(45:48));
        bump_ceil=bytes2float(db(49:52));
        staircase_ratio=bytes2float(db(53:56));
        stim_trial_flag=db(57);
        training_trial_freq=bytes2float(db(58:61));
        stim_freq=bytes2float(db(62:65));
        recenter_cursor_flag=db(66);
        tgt_radius=bytes2float(db(67:70));
        tgt_size=bytes2float(db(71:74));
        intertrial_time=bytes2float(db(75:78));
        penalty_time=bytes2float(db(79:82));
        bump_hold_time=bytes2float(db(83:86));
        ct_hold_time=bytes2float(db(87:90));
        ot_delay_time=bytes2float(db(91:94));
        bump_rate_skew=bytes2float(db(95:98));
        targets_during_bump_flag=db(99);
