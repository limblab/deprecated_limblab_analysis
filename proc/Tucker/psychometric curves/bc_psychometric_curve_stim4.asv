function alpha = bc_psychometric_curve_stim4(tt,tt_hdr,stimcode,plot_error,invert_error)
    ...
    reach_fit_stim_upper = sigmoid(dd,g_stim_upper(1),g_stim_upper(2),g_stim_upper(3),g_stim_upper(4));
    ...
    reach_fit_stim_lower = sigmoid(dd,g_stim_lower(1),g_stim_lower(2),g_stim_lower(3),g_stim_lower(4));
    ...
    g_no_stim_upper = get_ml_fit2(up_dirs_no_stim,num_left_reaches_no_stim_upper,number_reaches_no_stim_upper);
    reach_fit_no_stim_upper = sigmoid(dd,g_no_stim_upper(1),g_no_stim_upper(2),g_no_stim_upper(3),g_no_stim_upper(4));
        %get the parameters of the maximum likelyhood model of the psychometric
    %curve for no-stim reaches in the lower hemispace (180-360deg bumps)
    g_no_stim_lower = get_ml_fit2(down_dirs_no_stim,num_left_reaches_no_stim_lower,number_reaches_no_stim_lower);
    reach_fit_no_stim_lower = sigmoid(dd,g_no_stim_lower(1),g_no_stim_lower(2),g_no_stim_lower(3),g_no_stim_lower(4));
...
%     subplot(2,1,2),plot(dd, reach_fit_no_stim_lower, 'b-');
    ...
    %now get the family wise error rate between the stim curve and the non
    %stim curve
    %get matching sets of stim and non-stim points
    j=1;
    for i=1:length(dirs_stim)
        temp=find(dirs_no_stim==dirs_stim(i));
        if ~isempty(temp)
            proportion_stim_temp(j)=proportion_stim(i);
            number_reaches_stim_temp(j)=number_reaches_stim(i);
            proportion_no_stim_temp(j)=proportion_no_stim(temp);
            j=j+1;
        end
    end
    
    alpha = get_familywise_errors(proportion_no_stim_temp, round(proportion_stim_temp.*number_reaches_stim_temp), number_reaches_stim_temp);
   ...
   % subplot(2,1,1),plot(dirs,ps,'ko')
    ...
    axis([-10,370,-0.5,1.5])
    title(strcat('psychometric curves for no-stim and stim flag ',num2str(stimcode),' alpha level is: ',num2str(alpha)))
    ...
    axis([-10,370,0,10*(floor(max_reaches/10)+1)])
