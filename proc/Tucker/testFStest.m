...
Pending = InitPending();
Pending.Start_ts=-1
[tt Pending cyclenum] = dbFetchAndScrub('test',dbfmt,Pending,tt_hdr,0);
tt1(1:size(tt,1),jj:jj+size(tt,2)-1)= tt;
for ii=1:MAXCOUNT
    jj=size(tt1,2)+1;
    tt1(:,jj)=zeros(TT_HDR_SIZE,1);
    fprintf('ENTERING dbFetchAndScrub witn ii=%d, cyclenum=%d\n', ii, cyclenum)
    [tt, Pending, cyclenum] = dbFetchAndScrub('test',dbfmt, Pending, tt_hdr, cyclenum);
    tt1(1:size(tt,1),jj:jj+size(tt,2)-1)= tt; 
