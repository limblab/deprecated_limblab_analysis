function [avg_b, avg_dev, avg_stats, varargout] = glm_kin_TT(bdf, chan, unit,varargin)
    ...
    if nargout > 4
        %compute log likelihood of data given constant firing rate model
        lambda = sum(s)/length(s);
        varargout{2} = sum(log(lambda.^(s')) - lambda - log(factorial(s')));
    end
    if nargout > 5
