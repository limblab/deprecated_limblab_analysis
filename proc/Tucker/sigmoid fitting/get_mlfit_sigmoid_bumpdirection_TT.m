function phat=get_mlfit_sigmoid_bumpdirection_TT(TT,TT_hdr,mask)
    %takes in a trial table from the bumpdirection task which has been extended
    ...
    %get the data vector from the trial table
    data(:,1)=TT(:,TT_hdr.bump_angle);%angles
    data(:,2)=TT(:,TT_hdr.stim_trial);%stim condition

    data(:,3)=( TT(:,TT_hdr.trial_result)==2 & 90 <= TT(:,TT_hdr.bump_angle) &  TT(:,TT_hdr.bump_angle)<= 270 |...
            TT(:,TT_hdr.trial_result)==0 & -90 <= TT(:,TT_hdr.bump_angle) & TT(:,TT_hdr.bump_angle) <= 90 |...
            TT(:,TT_hdr.trial_result)==0 & 270 <= TT(:,TT_hdr.bump_angle) & TT(:,TT_hdr.bump_angle) <= 360  );%was the reach to the primary target or not
