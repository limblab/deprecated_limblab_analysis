    ...
    %% perform method specific selection of times of interest
        switch method
            ...
            case 'trials'
                ...
                outstruct=sample_between_timepoints(T,FR_timeseries,bdf,pos,vel,acc,force,dfdt,dfdtdt,pos_lag_data,vel_lag_data,acc_lag_data,force_lag_data,dfdt_lag_data,dfdtdt_lag_data,data_offset,num_lags,lags,method_opts,which_units);
            case 'bumps'
...
function outstruct=peak_value(x,bdf,target_onsets,num_lags,lags,FR_timeseries,pos,vel,acc,force,dfdt,dfdtdt,pos_lag_data,vel_lag_data,acc_lag_data,force_lag_data,dfdt_lag_data,dfdtdt_lag_data,data_window,data_offset,method_opts,which_units)
    ...
    %loop through targets finding the speed peaks
        ...
        for i=1:length(target_onsets)-1
            ...
            inds=find(x(:,1)>target_onsets(i) & x(:,1)<target_onsets(i+1));
            [maxVal,maxInd]=extrema(x(inds));
            if isempty(maxInd)
                warning('testAllTuning:NoMaxVel',strcat('Could not find a maxima for target onset# ',num2str(i)))
        ...
        test_end=bdf.TT(bdf.TT(:,7+2*targets_per_trial-1)==target_onsets(i),7+2*targets_per_trial);
        if ~isempty(test_end)
            inds=find(x(:,1)>target_onsets(i) & x(:,1)<test_end);
            [maxVal,maxInd]=extrema(x(inds));
            if isempty(maxInd)
                warning('testAllTuning:NoMaxVel',strcat('Could not find a maxima for target onset# ',num2str(i)))
                T=T(1:end-1);
        ...
        outstruct=sample_around_timepoints(x(T,1),FR_timeseries,bdf,pos,vel,acc,force,dfdt,dfdtdt,pos_lag_data,vel_lag_data,acc_lag_data,force_lag_data,dfdt_lag_data,dfdtdt_lag_data,data_offset,data_window,num_lags,lags,method_opts,which_units);
end

function outstruct=sample_around_timepoints(T,FR_timeseries,bdf,pos,vel,acc,force,dfdt,dfdtdt,pos_lag_data,vel_lag_data,acc_lag_data,force_lag_data,dfdt_lag_data,dfdtdt_lag_data,data_offset,data_window,num_lags,lags,method_opts,which_units)
