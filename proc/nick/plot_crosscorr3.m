function plot_crosscorr3(spikes,kinematics,numLags,states)
% plot_crosscorr3(spikes,kinematics,numLags,states)
%
...
for a = 1:numDims
    ...
    for b = 1:numDims
        for c = 1:2*numLags+1
            temp = corrcoef(dupKin(numLags+1:end,(a-1)*(2*numLags+1) + c),kinematics(1:end-numLags,b));
            cc0(c) = temp(1,2);
            temp = corrcoef(dupKin([false(numLags,1); states(1:end-numLags)],(a-1)*(2*numLags+1) + c),kinematics([states(1:end-numLags); false(numLags,1)],b));
            cc1(c) = temp(1,2);
            temp = corrcoef(dupKin([false(numLags,1); ~states(1:end-numLags)],(a-1)*(2*numLags+1) + c),kinematics([~states(1:end-numLags); false(numLags,1)],b));
            cc2(c) = temp(1,2);
        end
