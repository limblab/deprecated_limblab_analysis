% velocitySDFilterCompare2.m
%
% calculates FIR, TF, coherence, and PSD for POSITIONS derived from velocity predictions
%
...
for x = 1:length(pred.preddatabin); predpos(x+1) = predpos(x) + pred.preddatabin(x,dim)*0.05; end
for x = 1:length(pred.preddatabin); realpos(x+1) = realpos(x) + binnedData.velocbin(x+9,dim)*0.05; end
[B,A] = butter(4,0.01/10,'high');
predpos = filtfilt(B,A,predpos);
...
%                 [FIR{m}(dataset,movepost,dim,:),vaf,mcc] = filMIMO3(binnedData.velocbin(startindex:end,dim),pred.preddatabin(:,dim),num_lags,num_sides,1);
                [FIR{m}(dataset,movepost,dim,:),vaf,mcc] = filMIMO3(realpos',detrend(predpos)',num_lags,num_sides,1);
                if num_sides == 1
...
%                 [C{m}(dataset,movepost,dim,:),Cfreq{m}(dataset,:)] = mscohere(binnedData.velocbin(startindex:end,dim),pred.preddatabin(:,dim),window,[],[],Fs);
                [TF{m}(dataset,movepost,dim,:),TFfreq{m}(dataset,:)] = tfestimate(realpos,detrend(predpos),window,[],[],Fs);
                [C{m}(dataset,movepost,dim,:),Cfreq{m}(dataset,:)] = mscohere(realpos,detrend(predpos),window,[],[],Fs);
%                 h = spectrum.welch('Hamming', floor(length(pred.preddatabin(:,dim))/window));
                h = spectrum.welch('Hamming', floor(length(predpos)/window));
%                 PSDtemp = psd(h,pred.preddatabin(:,dim),'Fs',Fs);
                PSDtemp = psd(h,detrend(predpos),'Fs',Fs);
                PSDpred{m}(dataset,movepost,dim,:) = PSDtemp.data;
%                 h = spectrum.welch('Hamming', floor(length(binnedData.velocbin(:,dim))/window));
                h = spectrum.welch('Hamming', floor(length(realpos)/window));
%                 PSDtemp = psd(h,binnedData.velocbin(:,dim),'Fs',Fs);
                PSDtemp = psd(h,realpos,'Fs',Fs);
