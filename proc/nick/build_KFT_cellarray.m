function build_KFT_cellarray(dataFile,delayBins,startTime,endTime,genPlots)
% function build_KFT_cellarray(dataFile,delayBins,startTime,endTime,genPlots)
%
...
if isempty(binnedData.targets.corners)
    binnedData.targets.corners = VS_corners(binnedData, 8, 8, 2);
end
...
delay_spikes = [zeros(delayBins,size(spikes,2)); spikes];
...
for reach = 1:length(goals)-1
    ...
    reach_spikes{reach} = delay_spikes([(binnedData.timeframe >= goals(reach,1) & binnedData.timeframe < goals(reach+1,1)); false(delayBins,1)],:);
end

[A, C, Q, R] = train_kf(reach_state, reach_spikes);
...
if genPlots
    ...
    for x = 1:length(reach_spikes)
        ...
        [reach_pred_state, V, VV, loglik] = kalman_filter(reach_spikes{x}(:,:)', A, C, Q, 1*R, initState, initV);
    ...
    title(['KFT position prediction, delay = ' num2str(delayBins) ' vaf = ' num2str(mean([xpvaf ypvaf]))])
    xlabel('x (cm)')
    ylabel('y (cm)')
