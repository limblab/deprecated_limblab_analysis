...
for r_i = 16 % <--------------- for iterative testing (end at bottom)
...
% for i = 1:10
    ...
    [xpred{i}, Vpred{i}, VV{i}, loglik(i)] = kalman_filter(Z{i}', A, C, Q, Rnew, initx, initV); %%with target
...
title(['x Predictions - KFT VAF = ' num2str(vaf(1)) '; KF VAF = ' num2str(vaf0(1)) '; R* = ' num2str(Rmult)])
ylabel('Handle Position (cm)')
xlabel('Time (x)')
...
plot((startindex(1):length(testData.cursorposbin)).*bin, transitions(startindex(1):end)*20-20,'k*')
title(['y Predictions - KFT VAF = ' num2str(vaf(2)) '; KF VAF = ' num2str(vaf0(2)) '; R* = ' num2str(Rmult)])
ylabel('Handle Position (cm)')
xlabel('Time (x)')
