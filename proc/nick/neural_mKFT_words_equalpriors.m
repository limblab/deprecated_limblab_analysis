...
trainData.centergoals(:,3) = trainData.centergoals(:,3) - 33.5; %<----- THIS IS TO COMPENSATE FOR Y OFFSET... THERE IS NO X OFFSET IN THESE FILES, SHOULD GET THIS FROM BINNED DATA FILE OR DECLARE ABOVE
...
for i = 1:length(X)
% for i = 1:10
    if (X{i}(1,8)==0 && X{i}(1,9)==-33.5)
        target_goals = repmat([0 -33.5],8,1);
    else
...
%         initV{2} = zeros(length(initx{2}));
        for j = 1:8
%             initx{j} = [X{i}(1,1:7) VStargets(j,:)]'; 
            initx{j} = [X{i}(1,1:7) target_goals(j,:)]'; 
...
%         initV{2} = zeros(length(initx{2}));
        for j = 1:8
%             initx{j} = [xpred{i-1}(1:end-2,end); VStargets(j,:)'];
