...
delay = 0.100;
% window = 0.050; % in seconds (for spike averaging) should match training
% delay = 0.000;

bin = double(trainData.timeframe(2) - trainData.timeframe(1));
window_bins = floor(window/bin);
delay_bins = floor(delay/bin);
% window_bins = round(window/bin);
% delay_bins = round(delay/bin);
...
%     training_set(x,:) = mean(trainData.spikeratedata(x-(window_bins + delay_bins-1):x-(delay_bins),:));
...
%     test_set(x,:) = mean(testData.spikeratedata(x-(window_bins + delay_bins-1):x-(delay_bins),:));
...
%     if (endindex(i) - startindex(i) == 1)
%     xpred{i} = xpred{i-1}(:,end);
%     Vpred{i} = Vpred{i-1}(:,end);
%     VV{i} = VV{1,i-1}(:,end);
%     loglik(i) = loglik(i-1);
%     xpred0{i} = xpred0{i-1}(:,end);
%     Vpred0{i} = Vpred0{i-1}(:,end);
%     VV0{i} = VV0{i-1}(:,end);
%     loglik0(i) = loglik0(i-1);
%     else
