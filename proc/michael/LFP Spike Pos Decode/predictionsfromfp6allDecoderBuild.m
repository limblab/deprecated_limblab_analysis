...
%% Notch filter for 60 Hz noise - REMOVED RDF 03/15/2011
...
for i=1:numbins
	...
	% SECOND DETREND REMOVED RDF 03/15/2011
%     tmp=detrend(tmp);
    tmp=win.*tmp;
...
Pmat=tfmat(2:length(freqs),:,:).*conj(tfmat(2:length(freqs),:,:))*0.75;   
...
Pmean=shiftdim(filter(ones(1,wsz)/wsz,1,Pmean),1);
PA=10*(log10(Pmat)-log10(Pmean));
clear Pmat
