...
for i = 1:length(DaysNames)
    ...
    for l=1:length(MATfiles)
        ...
        save([sname,'spikes tik velpred ',num2str(binlen),'ms bins lambda',num2str(lambda),' Poly',num2str(Poly),'.mat'],...%<-Filename
        'v*','y*','r*','x','H','P','neuronIDs');

        clear v* y* x r* H bdf P...
              words cells
              
        close all

    end
    clear H Hbest featind
