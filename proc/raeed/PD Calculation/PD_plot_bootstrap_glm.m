function PD_plot_bootstrap_glm(varargin)
...
if length(varargin)<3
    ...
    %this is not a boolean. It is passed to glm_pds, which uses the value
    %to determine how unsorted trials are handled
end
...
model='posvel';
[pds, CI_low, CI_high, moddepth] = glm_bootstrap(bdf,include_unsorted,model);
...
%% make deselected channel pds, CIs and moddepth zero.
% deselected_chan = [96];
    
%pds(find(ismember(u1,deselected_chan))) = NaN ; 
%CI_low(find(ismember(u1,deselected_chan))) = NaN ; 
%CI_high(find(ismember(u1,deselected_chan))) = NaN ;
...
if plot_histogram
    ...
    figure('name','95% CI'); 
    hist(abs(CI_high(~isnan(CI_high))-CI_low(~isnan(CI_low)))*180/pi,5:10:360) % channels that have been deselected or eliminated in another way have NaN as PD, CI's and moddepths
    xlim([0,360])
    xlabel('degrees')
    ylabel('PD counts')
