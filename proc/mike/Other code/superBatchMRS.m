function superBatchMRS(animal,dateNumber)
...
elseif nargin==1
    ...
    [CEBorPLX,remoteFolder,~]=getDataByDateMRS(animal);
else
    % can run by remote, if we ever figure that out.
    [CEBorPLX,remoteFolder,~]=getDataByDateMRS(animal,dateNumber);
end
...
if strcmp(CEBorPLX,'ceb')
    ...
    batch_get_plexon_dataMRS % runs as script.  uses PathName
    %batch_buildLFPpositionDecoderMRS <- Use for LFPs
    MRSbatch_runpredspikes % <- Use for spikes
end
