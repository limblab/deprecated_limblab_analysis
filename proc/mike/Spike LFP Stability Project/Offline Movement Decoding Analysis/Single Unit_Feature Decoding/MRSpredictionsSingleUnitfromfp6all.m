function [vaf,vmean,vsd,y_test,y_pred,varargout] = MRSpredictionsSingleUnitfromfp6all(sig, signal, numfp, ...
...
if ~exist('featMat','var')
    ...
    if length(t)>numbins
        ...
        numbins = length(t); %MRS added 6/18/2013
    end
    ...
    PB = featMat; %PB should be input as a 2D matrix (rows = time, columns = features)
    q = ones(1,size(PB,1));
    t = 1:.001:size(PB,1);
...
for i = 1:size(x,2)
    ...
    if ~exist('H','var')
        ...
        [y_pred{i},xtnew{i},ytnew{i}] = predMIMO3(x_test,H{i},numsides,binsamprate,y_test);
        
    elseif exist('H','var') && length(H) < i
        ...
        if size(H,1) > 1
            ...
            for j = 1:size(H,2)
                ...
                if isempty(H{i,j})
                    ...
                    [y_pred{i,j},xtnew{i},ytnew{i}] = predMIMO3(x_test,H{i,j},numsides,binsamprate,y_test);
                end
    ...
    if size(H,1) > 1
        for k = 1:size(y_pred,2)
            ...
            if isempty(y_pred{i,k})
                ...
                for j=1:size(y,2)
                    r{i,j}=corrcoef(y_pred{i,k}(:,j),ytnew{i}(:,j));
                    
                    if size(r{i,j},2)>1
                        r2(i,j,k)=r{i,j}(1,2)^2;
                    else
                        r2(i,j,k)=r{i,j}^2;
                    end
                end
