...
% This is because of the manual way that the 10 lags has to be re-created
% offline using SVD, Offlinelags should be numlags/10 
numsides = 1;
lambda   = 1;
...
TestRandH = 0;
LoadFeatMat = 0;
startind = 1 % MSP Control (Mini/Chewie)
% startind = 52; % Mini
% startind = 162; % Chewie
...
for q = 13:length(FileList)
    ...
    if spikes == 1
        ...
        for i = 1:length(cells{q})
            if cells{q}(i,1) ~= 0
                for j = 1:length(neuronIDs)
                    if cells{q}(i,1) == neuronIDs(j,1)
                        ts = get_unit(bdf, cells{q}(i, 1), cells{q}(i, 2));
                        b = train2bins(ts, t);
                        x(:,j) = b;
                        break
                    end
...
%                 end
                [n_para{q}, n_perp{q}, H_para_AllLag{q}, H_perp_AllLag{q},...
                    H_para_Allalpha{q}, H_perp_Allalpha{q}, H_perp_3_4{q}] = ...
            ...
            if q == length(FileList)
                for fi = length(FileList)-5:length(FileList); % Index for test file
                    ...
                    for ti = startind:length(H_para_AllLag) % Index for decoder
...
%                                 [y_pred_perp_all,xtnew,ytnew_perp] = predMIMO3(n_perp_all,H_perp_Allalpha{ti},numsides,binsamprate,y{fi});
                                ...
                                n_perp_3_4 = reshape(n_perp{fi}(:,:,1:2),size(n_perp{fi},1),size(n_perp{fi},2)*2);
                                [y_pred_perp_3_4,xtnew,ytnew_perp_3_4] = predMIMO3(n_perp_3_4,H_perp_3_4{ti},numsides,binsamprate,y{fi});
...
%                                 
                                r_perp_3_4{fInd,ti,j}=corrcoef(y_pred_perp_3_4(:,j),ytnew_perp_3_4(:,j));
%                                 
...
%                                 R2_perp_All(fInd,ti,j) = r_perp_all{fInd,ti,j}(1,2)^2;
                                ...
                                R2_perp_3_4(fInd,ti,j) = r_perp_3_4{fInd,ti,j}(1,2)^2;
                            end
