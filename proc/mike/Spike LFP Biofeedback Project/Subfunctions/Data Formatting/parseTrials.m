...
%                     Trial.FPMaxV{t,B} = fp(TrialMVIndexFP(j)-2000:TrialMVIndexFP(j)+2000,1,B);
                    Trial.FPend{t,B} = fp(TrialStartIndexFP(j)-2000:TrialEndIndexFP(j)+2000,1,B);
                end
            else
                j = j+1;
                t = t+1;
...
%             MaxVTrial_SpikeTimes = ts((ts >= MaxVelT(t)-2 & ts<=MaxVelT(t)+2)) - (MaxVelT(t));
            EndTrial_SpikeTimes = ts((ts >= TimeGo-2 & ts<=TimeEnd+2)) - (TimeEnd);
...
%                     Trial.Incomplete_FPMaxV{t,B} = fp(TrialMVIndexFP(j)-2000:TrialMVIndexFP(j)+2000,1,B);
                    Trial.Incomplete_FPend{t,B} = fp(TrialStartIndexFP(j)-2000:TrialEndIndexFP(j)+2000,1,B);
                end
            else
                j = j+1;
                t = t+1;
...
%                     Trial.Fail_FPMaxV{t,B} = fp(TrialMVIndexFP(j)-2000:TrialMVIndexFP(j)+2000,1,B);
                    Trial.Fail_FPend{t,B} = fp(TrialStartIndexFP(j)-2000:TrialEndIndexFP(j)+2000,1,B);
                end
            else
                j = j+1;
                t = t+1;
