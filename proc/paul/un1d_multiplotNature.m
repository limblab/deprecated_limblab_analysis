function h = un1d_multiplotNature(monkeyName, brainArea,dateStamp1, setRange1,dateStamp2, setRange2, dateStamp3, setRange3,targetDir)
...
setRange = [setRange1 setRange2 setRange3];
%% operate over each set in the specified range
for si=1:length(setRange)
    
    if si<=length(setRange1)

        %    load bdf and trial table for first set
        fn_bdf = ['bdf/bdf_' monkeyName '_' brainArea '_' dateStamp1 '_UN1D_00' int2str(setRange(si)) '.mat'];
        fn_tt  = ['tt/tt_'  monkeyName '_' brainArea '_' dateStamp1 '_UN1D_00' int2str(setRange(si)) '.mat'];
        load(fn_bdf);
        load(fn_tt);
    elseif  si<=length([setRange1 setRange2])
        %    load bdf and trial table for first set
        fn_bdf = ['bdf/bdf_' monkeyName '_' brainArea '_' dateStamp2 '_UN1D_00' int2str(setRange(si)) '.mat'];
        fn_tt  = ['tt/tt_'  monkeyName '_' brainArea '_' dateStamp2 '_UN1D_00' int2str(setRange(si)) '.mat'];
        load(fn_bdf);
        load(fn_tt);
    elseif  si<=length([setRange1 setRange2 setRange3])
        %    load bdf and trial table for first set
        fn_bdf = ['bdf/bdf_' monkeyName '_' brainArea '_' dateStamp3 '_UN1D_00' int2str(setRange(si)) '.mat'];
        fn_tt  = ['tt/tt_'  monkeyName '_' brainArea '_' dateStamp3 '_UN1D_00' int2str(setRange(si)) '.mat'];
        load(fn_bdf);
        load(fn_tt);
    ...
    dtb_ts =  tt(1:end,1);    % databurst timestamps
    dtb_shifts =  tt(1:end,2);    % shift values for each trial
    dtb_feedback_sig =  tt(1:end,3);    % feedback condition for each trial
    dtb_centeron_ts =     tt(1:end,4);    % timestamp of center target
    dtb_outeron_ts = tt(1:end,5);          % timestamp of target drawn
    dtb_gocue_ts = tt(1:end,6);            % timestamp of go cue
    dtb_endcode_ts = tt(1:end,7);           % timestamp of trial complete
    dtb_end_codes = tt(1:end,8);        % code for trial complete
    ...
    figtitle = [fn_prefix 'Cursor Endpoint vs Shift.fig'];
    title(figtitle);
    hgsave(figtitle);
...
xlabel('Prior Lateral Shift (cm)');
ylabel('Hand Endpoint Lateral Error (cm)');
figtitle = ['figs/' fn_prefix(1:end-5) 'Multi: Hand Endpoint vs Shift.fig'];
title(figtitle);
hgsave(figtitle);
...
xlabel('Prior Lateral Shift (cm)');
ylabel('Cursor Endpoint Lateral Error (cm)');
figtitle = ['figs/' fn_prefix(1:end-5) 'Multi: Cursor Endpoint vs Shift.fig'];
title(figtitle);
hgsave(figtitle);
