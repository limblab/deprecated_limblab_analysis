...
binsize=0.050; %50 ms
time_range = [-4:binsize:4];
numtrials = size(kin.ts,1);
numbins = length(time_range);
movbuffer = 2; % buffer on either side of GO and END in seconds
% for each unit
for ui=1:numunits
     if mod(ui,4)==1
        figure(ui);
    ...
    for ti=1:numtrials
        ...
        spikesAligned = sortedunits(ui).ts(spikerange)-trange(1);
    ...
    xlim([-movbuffer 4]);
    title(['PSTH: (GO) Channel ' num2str(sortedunits(ui).id(1)) ', Unit ' num2str(sortedunits(ui).id(2))]);
