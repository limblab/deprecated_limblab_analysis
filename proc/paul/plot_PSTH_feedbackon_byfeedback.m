...
for ui=1:numunits
    ...
    for ti=1:numtrials
        ...
        for bi=1:numbins-1
           binnedSpikes(ti,bi) = length(find(spikesAligned>=time_bins(bi) & spikesAligned <=time_bins(bi+1)));
        end
    ...
    bar(time_bins',[PSTH_sm(ui,:); PSTH_lg(ui,:)]','histc');
    xlim([-2 2]);
