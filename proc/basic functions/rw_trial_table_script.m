...
% returns a table containing the key timestamps for all of
...
start_words =  bdf.words( bdf.words(:,2) == word_start &  bdf.words(:,1)>1.000, 1);
num_trials = length(start_words);

word_go = hex2dec('30');
go_cues =  bdf.words(bitand(hex2dec('f0'), bdf.words(:,2)) == word_go, 1);
go_codes=  bdf.words(bitand(hex2dec('f0'), bdf.words(:,2)) == word_go, 2)-word_go;

word_end = hex2dec('20');
end_words =  bdf.words( bitand(hex2dec('f0'), bdf.words(:,2)) == word_end, 1);
end_codes =  bdf.words( bitand(hex2dec('f0'), bdf.words(:,2)) == word_end, 2);
...
%add new trial table variables to bdf
bdf.TT=tt;
bdf.TT_hdr=tt_hdr;
if isfield(bdf.meta,'processed_with')
    ...
    bdf.meta.processed_with(end+1,:)={'rw_trial_table_script',date,hostname,username};
end

%clean up variables that used to be local variables:
clear tt
clear tt_hdr
clear start_time
clear num_targets
clear num_targets_attempted
clear x_offset
clear y_offset
clear tgt_size
clear go_codes
...
clear end_time
clear trial_result
clear skipped_trials
clear go_cue_idx
clear these_go_cues
clear these_go_codes
clear tgts_attempted
clear word_start
clear start_words
clear num_trials
clear word_go
