...
for j = 1:length(bump_magnitudes)
    temp_succ_1 = length(table(table(:,6)==bump_directions(1) &...
        table(:,3)==32 & table(:,7)==bump_magnitudes(j),1));
    temp_unsucc_1 = length(table(table(:,6)==bump_directions(1) &...
        table(:,3)==34 & table(:,7)==bump_magnitudes(j),1));
    temp_succ_2 = length(table(table(:,6)==bump_directions(2) &...
        table(:,3)==32 & table(:,7)==bump_magnitudes(j),1));
    temp_unsucc_2 = length(table(table(:,6)==bump_directions(2) &...
        table(:,3)==34 & table(:,7)==bump_magnitudes(j),1));
