% 'load_data.m' - load data, currently from human experiment.
% Dynamic model ANN fitting - use ANN to predict kinematics from EMG.
% 
...
file_name = 'David_2014-06-18_RW_EMG_002';
...
% Normalize EMG
emg_norm = zscore(emg_data);
% emg_norm = max(emg_norm,0);
% emg_norm = emg_norm./repmat(max(emg_norm),size(emg_norm,1),1);
emg_norm = bsxfun(@minus,emg_norm,min(emg_norm));%emg_norm - repmat(min(emg_norm),size(emg_norm,1),1);
