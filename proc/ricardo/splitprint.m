...
%   -dpng -r600 option to 'splitprint.png'.
...
saveas(gcf, 'e:/temp.fig', 'fig');

%default values, modify to set your defaults
fig = gcf;
printtofile = logical(1); %print to file by default, if this is 0, just splits figure
filename = 'splitprint';
types = {{'text';'line'}, {'patch';'surface';'image';'area'}};
fmts = {'-depsc2','-dpng'};
alignmarks = logical(0);
visaxes = logical([1 0]);
options = {'-adobecset',{'-r600','-cmyk'}}; % default eps for first fig; 600 dpi for second
% RGB OPTION:
% options = {'-adobecset',{'-r600'}}; % default eps for first fig; 600 dpi for second
...
%check types
actualtypes = {'image';'light';'line';'patch';'surface';'text';'area'};
...
for i = 1: nfigs,
    ...
    % NEW: 20050612: Solution Number: 1-18YSG, copyobj no longer reverses
    % order starting with R14SP1.
