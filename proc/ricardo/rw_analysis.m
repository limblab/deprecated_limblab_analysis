...
filename = 'Tiki_2011-05-17_RW_001-s_thres2';
extension = '.nev';
...
dm = zeros(length(ul),1);
speed_comp = zeros(length(ul),1);
...
for i = 1:length(ul)
    ...
    pds(i,:) = pd;
    speed_comp(i) = mean(fr);
    dm(i) = (max(fr)-min(fr));
end
% %%
% figure
% ul_full = reshape([bdf.units.id],2,[])';
% for i = 1:length(ul)
%     clf
%     unit_idx = find(ul_full(:,1)==ul(i,1) & ul_full(:,2)==ul(i,2));
%     [tempc, ia, ib] = intersect(bdf.vel(:,1),bdf.units(unit_idx).ts);
%     plot(bdf.vel(ia,2),bdf.vel(ia,3),'.')
%     hold on
%     plot(0,0,'.r')
%     xlim([-100 100])
%     ylim([-100 100])
