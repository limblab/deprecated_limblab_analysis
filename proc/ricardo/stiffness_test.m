...
%     addpath('C:\Documents and Settings\limblab\Desktop\s1_analysis\')
    ...
    dataFile = 'D:\Data\bump_stim_forces_test_006.plx';
...
first_trial = find(bdf.words(:,2)==20 | bdf.words(:,2)==21,1);
last_trial = find(bdf.words(:,2)>=32 & bdf.words(:,2)<=36,1,'last');

temp_words = bdf.words(first_trial:last_trial,:);

trials.start_time = temp_words(temp_words(:,2)==20 | temp_words(:,2)==21,1);
trials.end_time = temp_words(temp_words(:,2)>=32 & temp_words(:,2)<=36);
...
figure;
for i = 1:length(bump.force)
plot(bump.force{i}(:,1)-bump.force{i}(1,1),bump.force{i}(:,3)); hold on
end
...
for i = 1:length(bump.time)
    ...
    %     hold on
        ...
        plot(time,bump.pos{i}(:,3)-bump.pos{i}(1,3))
        ylabel('y (cm)')
        ylim([-6 6])
        xlim([-.5 .5])
