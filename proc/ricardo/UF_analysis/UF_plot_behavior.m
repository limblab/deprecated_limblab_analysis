function UF_plot_behavior(UF_struct,bdf)
...
%             area(temp_t,temp_std,'FaceColor',min(UF_struct.colors_field_bias(iBias,:)*1,[1 1 1]),'LineStyle','none')   
            area(temp_t,temp_sem,'FaceColor',min(UF_struct.colors_field_bias(iBias,:)*1,[1 1 1]),'LineStyle','none')   
...
figure
hold on
x_force_pre_bump = mean(UF_struct.x_force(:,UF_struct.t_axis<0),2) +  mean(bdf.force(:,2));
y_force_pre_bump = mean(UF_struct.y_force(:,UF_struct.t_axis<0),2) +  mean(bdf.force(:,3));
