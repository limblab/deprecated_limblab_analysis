...
%     params.movie_range = [10 110];
    headers = DCO.BMI.params.headers;
       
    idx_start_emg = find(DCO.BMI.data(:,strcmp(headers,'t_bin_start'))>=params.movie_range(1),1,'first');
    idx_end_emg = find(DCO.BMI.data(:,strcmp(headers,'t_bin_start'))<=params.movie_range(2),1,'last');
    
%     idx_start_hc = find(bdf_hc.pos(:,1)>=params.movie_range(1),1,'first');
    
    ct_radius = 1.8;

    ot_inner_radius_emg = DCO.trial_table(1,DCO.table_columns.outer_target_radius)-.5*DCO.trial_table(1,DCO.table_columns.outer_target_thickness);
    ot_outer_radius_emg = DCO.trial_table(1,DCO.table_columns.outer_target_radius)+.5*DCO.trial_table(1,DCO.table_columns.outer_target_thickness);
    ot_span_emg = DCO.trial_table(1,DCO.table_columns.outer_target_span);
...
%         'LineWidth',2); 
%     h_force_limits_hc = plot(DCO.BMI.data(idx_start_emg,strcmp(headers,'pred_x')),DCO.BMI.data(idx_start_emg,strcmp(headers,'pred_y')),'.k',...
%         'LineWidth',2); 
%     h_text_hc = text(-10,-10,'');
%     
%     for iFrame = 1:(idx_end_emg-idx_start_emg)   
%         hc_idx = find(bdf_hc.pos(:,1)>=DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'t_bin_start')),1,'first');
% 
...
%         
%         last_word_hc = bdf_hc.words(find(bdf_hc.words(:,1)<DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'t_bin_start')),1,'last'),2);
%         set(h_text_hc,'String',{num2str(last_word_hc);[num2str(DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'t_bin_start'))) ' s']})
%         trial_table_idx_hc = find(DCO_hc.trial_table(:,DCO_hc.table_columns.t_ct_hold_on)<=DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'t_bin_start')),1,'last');
...
%         
%         drawnow
%         pause(diff(DCO.BMI.data(idx_start_emg+iFrame+[0 1],strcmp(headers,'t_bin_start'))))
%     end
  %%  
    figure
%     subplot(121)
    ...
    h_upper_arm_emg = plot([DCO.BMI.data(idx_start_emg,strcmp(headers,'sh_x')) DCO.BMI.data(idx_start_emg,strcmp(headers,'el_x'))],...
        [DCO.BMI.data(idx_start_emg,strcmp(headers,'sh_y')) DCO.BMI.data(idx_start_emg,strcmp(headers,'el_y'))],'-k','LineWidth',2);
    h_lower_arm_emg = plot([DCO.BMI.data(idx_start_emg,strcmp(headers,'el_x')) DCO.BMI.data(idx_start_emg,strcmp(headers,'pred_x'))],...
        [DCO.BMI.data(idx_start_emg,strcmp(headers,'el_y')) DCO.BMI.data(idx_start_emg,strcmp(headers,'pred_y'))],'-k','LineWidth',2);  
    h_hand_emg = plot(DCO.BMI.data(idx_start_emg,strcmp(headers,'pred_x')),DCO.BMI.data(idx_start_emg,strcmp(headers,'pred_y')),'.b',...
        'MarkerSize',20); 
    h_force_emg = plot(DCO.BMI.data(idx_start_emg,strcmp(headers,'pred_x')),DCO.BMI.data(idx_start_emg,strcmp(headers,'pred_y')),'-b',...
        'LineWidth',2); 
    h_force_limits_emg = plot(DCO.BMI.data(idx_start_emg,strcmp(headers,'pred_x')),DCO.BMI.data(idx_start_emg,strcmp(headers,'pred_y')),'.k',...
        'LineWidth',2); 
    ...
    for iFrame = 1:(idx_end_emg-idx_start_emg)    
        ...
        x_hand_emg = DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'pred_x'));
        y_hand_emg = DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'pred_y'));
        
        x_force_emg = x_hand_emg + [0 DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'F_x'))];
        y_force_emg = y_hand_emg + [0 DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'F_y'))];
        
        set(h_hand_emg,'XData',x_hand_emg)
        set(h_hand_emg,'YData',y_hand_emg)
        set(h_upper_arm_emg,'XData',[DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'sh_x')) DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'el_x'))])
        set(h_upper_arm_emg,'YData',[DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'sh_y')) DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'el_y'))])
        set(h_lower_arm_emg,'XData',[DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'el_x')) x_hand_emg])
        set(h_lower_arm_emg,'YData',[DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'el_y')) y_hand_emg])        
        set(h_force_emg,'XData',x_force_emg)
        set(h_force_emg,'YData',y_force_emg)
        
        last_word_emg = bdf.words(find(bdf.words(:,1)<DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'t_bin_start')),1,'last'),2);
        set(h_text_emg,'String',{num2str(last_word_emg);[num2str(DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'t_bin_start'))) ' s']})
        trial_table_idx_emg = find(DCO.trial_table(:,DCO.table_columns.t_ct_hold_on)<=DCO.BMI.data(idx_start_emg+iFrame,strcmp(headers,'t_bin_start')),1,'last');

        ot_angle_emg = DCO.trial_table(trial_table_idx_emg,DCO.table_columns.outer_target_direction);
 
        force_limits_emg = DCO.trial_table(trial_table_idx_emg,DCO.table_columns.target_force) *...
            [1-DCO.trial_table(trial_table_idx_emg,DCO.table_columns.target_force_range) ...
            1+DCO.trial_table(trial_table_idx_emg,DCO.table_columns.target_force_range)];
