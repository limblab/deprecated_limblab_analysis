function DCO_plot_STAEMG(data_struct,params)
DCO = data_struct.DCO;
bdf = data_struct.bdf;

if isfield(bdf,'units')

    DCO_struct.t_axis = bdf.pos(:,1);
    ...
    for iEMG = 1:size(bdf.emg.data,2)-1    
        emg_filtered(:,iEMG)=abs(filtfilt(b,a,double(bdf.emg.data(:,iEMG+1))));  
    end
    plot_emgs = find(max(emg_filtered)./mean(emg_filtered) > 20);
%%
    for iUnit = 1:size(units,1)    
        unit_idx = find(all_chans(:,1)==units(iUnit,1) & all_chans(:,2)==units(iUnit,2));
%         try
%             electrode = params.elec_map(find(params.elec_map(:,3)==all_chans(unit_idx,1)),4);
%         catch  
%             temp = reshape([params.elec_map{:}]',4,[])';
%             temp = [temp{:,3}]';
%             temp = find(temp==all_chans(unit_idx,1));
%             electrode = params.elec_map{temp}{4};
%         end
%         electrode = DCO_struct.elec_map(find(DCO_struct.elec_map(:,3)==all_chans(unit_idx,1)),4);  
...
%         figTitles{end+1} = [num2str(electrode,'%1.2d') '-' num2str(units(iUnit,2))];
        figure
        for iPlot = 1:length(plot_emgs)
            iEMG = plot_emgs(iPlot);
            ...
            subplot(length(plot_emgs),1,iPlot)
...
%         gca = axes;
% %         h = title({DCO_struct.DCO_file_prefix;...
...
%     if save_figs
%         save_figures(figHandles,DCO_struct.DCO_file_prefix,DCO_struct.datapath,'STAEMG',figTitles)
%     end
end