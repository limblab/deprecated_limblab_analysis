function [xdot,torques] = arm_dynamics(t,x,arm_params)
...
M = [J(1) + J(2) + m(2)*l(1)^2 + 2*m(2)*l(1)*d(2)*cos(theta(2)) ...
...
if ~arm_params.monkey_control
    ...
    T_feedforward(2) = -m(2)*l(1)*cos(theta(2))*theta_ref_dot_dot(2) +...
        m(2)*l(1)*d(2)*sin(theta(2))*((theta_dot(1)+theta_dot(2))^2);
...
T_feedback_vel = -[b*theta_dot(1); b*theta_dot(2)];
% T_musc = T_musc - T_feedback_vel;
...
torques = [T_feedback T_feedforward T_musc' T_endpoint' T_feedback_vel'];

xdot(1:2,1) = theta_dot;
xdot(3:4,1) = M\(C + T_musc + T_endpoint + T_feedback_vel);
