...
unicount = zeros(length(chanunit),1);
lendata = zeros(length(chanunit),1);
meanisi = zeros(length(chanunit),1);
stdisi = zeros(length(chanunit),1);
cvisi = zeros(length(chanunit),1);
...
for i=1: length(starts)
    ...
    while rt<ends(i)+timeaft
        ...
        xvel = data.vel(find(data.vel(:,1)>=rt,1,'first'):find(data.vel(:,1)<=rt+timeaft,1,'last'),2);
%         xvel = mean(xvel);
        yvel = data.vel(find(data.vel(:,1)>=rt,1,'first'):find(data.vel(:,1)<=rt+timeaft,1,'last'),3);
%         yvel = mean(yvel);
        
        yud=yud+1;
        direction(yud)= atan2(yposvect,xposvect);
        speed(yud) = mean(sqrt(xvel.^2+yvel.^2));
...
for r=1:size(cha_uni,1)
    ...
    mag = zeros(1,length(cha_uni));
    for x = 1:size(cha_uni,1)
        mag(x) =mean(bootstrapPDS{x}(:,2));
    end;
    allPDs = zeros(length(cha_uni),12);
    allPDs(:,1:2)= cha_uni;
    allPDs(:,3:5)= PDMatrix;
    allPDs(:,6)= mag;
    allPDs(:,7)= unicount;
