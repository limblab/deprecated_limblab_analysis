...
resultpath = 'D:\Ricardo\Miller Lab\Bump choice results\DetectionB results\';
[datapath filelist] = BC_detectionB_experiment_list();
...
for file_no = 1:length(filelist)
    ...
    stim_pds = filelist(file_no).pd;
        if ~exist([datapath filename '.mat'],'file')    
            ...
            trial_table = BC_detectionB_trial_table([datapath filename]);    
        ...
        response_time = trial_table(:,3)-trial_table(:,2);
        
        correct = trial_table(:,4)==32;
        [correct_fit fit_stats] = fit(trial_table(:,2),correct,f_linear);
        percentage_rewards = sum(trial_table(:,4)==32)/length(trial_table) 
        percentage_aborts =  sum(trial_table(:,4)==33)/length(trial_table) 
        percentage_incompletes = sum(trial_table(:,4)==35)/length(trial_table) 
        ...
        % timing plot
        movement_time = [trial_table(:,1) trial_table(:,3)]-repmat(trial_table(:,2),1,2);
        [temp sort_idx] = sort(movement_time);
        movement_time_sorted = movement_time(sort_idx(:,1),:);
        trial_table_sorted = trial_table(sort_idx(:,1),:);
        
        figure; 
        plot(movement_time_sorted(trial_table_sorted(:,4)==32,:)',...
            [sort_idx(trial_table_sorted(:,4)==32) sort_idx(trial_table_sorted(:,4)==32)]','b')
        hold on;
        plot(movement_time_sorted(trial_table_sorted(:,4)==33,:)',...
            [sort_idx(trial_table_sorted(:,4)==33) sort_idx(trial_table_sorted(:,4)==33)]','r')
        plot(movement_time_sorted(trial_table_sorted(:,4)==35,:)',...
            [sort_idx(trial_table_sorted(:,4)==35) sort_idx(trial_table_sorted(:,4)==35)]','k')
        
        figure; hist(movement_time_sorted(trial_table_sorted(:,4)==32,1),30)
        figure; hist(movement_time_sorted(trial_table_sorted(:,4)==33,1),30)
