...
for i=1:length(binnedData.trialtable)
    ...
    patch([start(i)*.05 stop(i)*.05 stop(i)*.05 start(i)*.05],[0.4 0.4 60 60],'k','FaceAlpha',0.1,'EdgeAlpha',0);
...
y1=binnedData.emgdatabin(:,7);
h1 = plot(x,y1,'Color','r','LineWidth',1.5);
ylim([0 60])


y2=binnedData.emgdatabin(:,8);
h2=plot(x,y2,'Color','m','LineWidth',1.5);
ylim([0 60])

y3=binnedData.emgdatabin(:,9);
h3 = plot(x,y3,'Color','y','LineWidth',1.5);
ylim([0 60])

y4=binnedData.emgdatabin(:,10);
h4 = plot(x,y4,'Color','b','LineWidth',1.5);
ylim([0 60])

y5=binnedData.emgdatabin(:,11);
h5 = plot(x,y5,'Color','c','LineWidth',1.5);
ylim([0 60])

y6=binnedData.emgdatabin(:,12);
h6 = plot(x,y6,'Color','g','LineWidth',1.5);
ylim([0 60])

legend([h1 h2 h3 h4 h5 h6], binnedData.emgguide(7:12));
