...
function [HybridFinal AlteredIsoFinal AlteredWMFinal] = makeHybridFileFixedScaleMeans(File1, File2)  %(iso , wm)
...
%File1 = cutBinnedDataFile(File1, 18001, length(File1.timeframe))
%File2 = cutBinnedDataFile(File2, startCut, stopCut)
...
%TaskFlag
hybridData.taskflag(newIndexH : newIndexH+600-1,:) = 0;
hybridData.taskflag(newIndexH +600: newIndexH+1200-1 ,: ) = 1;
...
for i=1:length(File1.emgdatabin(1,:))
    hybridData.scale(i,1) = mean(File1.emgdatabin(:,i))/(mean(File2.emgdatabin(:,i)));
end
