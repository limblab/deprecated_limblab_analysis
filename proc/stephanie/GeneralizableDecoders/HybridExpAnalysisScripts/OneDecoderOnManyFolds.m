function [R2, varargout] = OneDecoderOnManyFolds(filter,binnedData,foldlength,varargin)
...
for i=1:nfold
    ...
    for ind=1:length(Pred)
        ...
        MeanVaf(i,:)  = 1 - sum( (Pred-Act).^2 ) ./ sum( (Act - repmat(mean(Act),size(Act,1),1)).^2 );
        

end


MeanVaf = reshape(MeanVaf,1,[]);
varargout = {filter, PredData, vaf, mse, ActSignalsTrunk MeanVaf};
