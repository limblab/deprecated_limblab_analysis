...
foldlengthPC = 60;
[~, ~,~, ~,~,~, HonI_PC_vaf, HonI_PC_mse, ~, HonI_predPCs] = PeriodicR2_SNwPCs(Hmodel,IsoTest,foldlengthPC);
[~, ~,~, ~,~,~, IonI_PC_vaf, IonI_PC_mse, ActualI_PCs, IonI_predPCs] = PeriodicR2_SNwPCs(IsoModel,IsoTest,foldlengthPC);
[~, ~,~, ~,~,~, WonI_PC_vaf, WonI_PC_mse, ~, WonI_predPCs] = PeriodicR2_SNwPCs(WmModel,IsoTest,foldlengthPC);

%|
[~, ~,~, ~,~,~, HonW_PC_vaf, HonW_PC_mse, ~, HonW_predPCs] = PeriodicR2_SNwPCs(Hmodel,WmTest,foldlengthPC);
[~, ~,~, ~,~,~, WonW_PC_vaf, WonW_PC_mse, ActualW_PCs, WonW_predPCs] = PeriodicR2_SNwPCs(WmModel,WmTest,foldlengthPC);
[~, ~,~, ~,~,~, IonW_PC_vaf, IonW_PC_mse, ~, IonW_predPCs] = PeriodicR2_SNwPCs(IsoModel,WmTest,foldlengthPC);

if SpringFile == 1
    [~, ~,~, ~,~,~, HonS_PC_vaf, HonS_PC_mse, ~, HonS_predPCs] = PeriodicR2_SNwPCs(Hmodel,SprTest,foldlengthPC);
    [~, ~,~, ~,~,~, WonS_PC_vaf, WonS_PC_mse, ~, WonS_predPCs] = PeriodicR2_SNwPCs(WmModel,SprTest,foldlengthPC);
    [~, ~,~, ~,~,~, IonS_PC_vaf, IonS_PC_mse, ~, IonS_predPCs] = PeriodicR2_SNwPCs(IsoModel,SprTest,foldlengthPC);
    [~, ~,~, ~,~,~, SonS_PC_vaf, SonS_PC_mse, ActualS_PCs, SonS_predPCs] = PeriodicR2_SNwPCs(SprModel,SprTest,foldlengthPC);
end
