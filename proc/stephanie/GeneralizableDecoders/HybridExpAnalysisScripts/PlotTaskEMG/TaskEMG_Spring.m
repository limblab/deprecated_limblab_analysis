function TaskEMG_Spring(binnedData,emg1,emg2,emg3,emg4,save,foldername, filename)
% TaskEMG_Spring(SprBinned,'FCU', 'FCR', 'ECU', 'ECR');
% TaskEMG_Spring(SprBinned,'FDS', 'FDP', 'ECU', 'ECR');

% This function makes a figure that shows the difference in EMG for the
% spring task. It produces a subplot(2,1). The top plot has EMGs
...
%figure;
ax1 = subplot(3,1,2); hold on; %Make top subplot for the EMG data
title(strcat(['Spring ',binnedData.meta.datetime(1:9)]));
