function [R2, varargout] = PeriodicR2_SNwPCs_weightingPCs(filter,binnedData,foldlength,varargin)
...
for i=1:numPredSigs
    if isfield(binnedData,'emgdatabin')
        if ~isempty(binnedData.emgdatabin)
            if length(binnedData.emgguide) == length(PredData.outnames)
                if  all(strcmp(nonzeros(char(binnedData.emgguide(1,:))),nonzeros(char(PredData.outnames(i,:)))))
    ...
    if isfield(binnedData,'forcedatabin')
        if ~isempty(binnedData.forcedatabin)
            if length(binnedData.forcelabels) == length(PredData.outnames)
                if all(strcmp(binnedData.forcelabels,PredData.outnames)) % all(strcmp(nonzeros(binnedData.forcelabels(1,:)),nonzeros(PredData.outnames(i,:))))
    ...
    if isfield(binnedData,'cursorposbin')
        if ~isempty(binnedData.cursorposbin)
            if length(binnedData.cursorposlabels) == length(PredData.outnames)
                if all(strcmp(binnedData.cursorposlabels,PredData.outnames)) % all(strcmp(nonzeros(binnedData.cursorposlabels(1,:)),nonzeros(PredData.outnames(i,:))))
    ...
    if isfield(binnedData,'velocbin')
        if ~isempty(binnedData.velocbin)
            if length(binnedData.veloclabels) == length(PredData.outnames)
                if all(strcmp(binnedData.veloclabels,PredData.outnames)) % all(strcmp(nonzeros(binnedData.veloclabels(1,:)),nonzeros(PredData.outnames(i,:))))
...
% Initialize R2, vaf, mse
numOfRows = length(ActSignalsTrunk(1,:));
R2 = zeros(nfold,numOfRows); vaf = zeros(nfold,numOfRows); mse = zeros(nfold,numOfRows);

% Get PC transformation from the actual EMGs
 [Coeff,PCs,Latent,Tsquared,Explained,Mu] = pca(ActSignalsTrunk);
 fullPredPCs = (PredData.preddatabin-repmat(mean(PredData.preddatabin),length(PredData.preddatabin(:,1)),1))*Coeff;
for i=1:nfold
    ...
    PCsnippet = PCs(DataStart:DataEnd,:);
    ...
    predPCs = (Pred-repmat(mean(Pred),length(Pred(:,1)),1))*Coeff;
    PC_vaf(i) = calculateVAF(predPCs(:,1),PCsnippet(:,1));  
    for a=1:length(PCs(1,:))
        allPCs_vaf(a,i) = calculateVAF(predPCs(:,a),PCsnippet(:,a));
    end
    PC_mse(i) =  mean((predPCs(:,1)-PCsnippet(:,1)).^2);

end

meanVAFs = mean(allPCs_vaf');
negInd = find(meanVAFs<0);
meanVAFs(negInd)=0;
weightedAveVAF = (meanVAFs*Latent)/(sum(Latent));
EigenVAF = norm(meanVAFs);

varargout = {filter, PredData, vaf, mse, ActSignalsTrunk, PC_vaf, PC_mse PCsnippet predPCs weightedAveVAF EigenVAF};
