function [R2, varargout] = mfxval_wmultivariateVAF(binnedData, options)
...
%     R2(i+1,:,1) = CalculateR2(TestSigs(round(fillen/binsize):end,:),PredData.preddatabin)';
    ...
    TestSigsRightLength=TestSigs(round(fillen/binsize):end,:);
   for ind=1:length(PredData.preddatabin)
        PredNorm(ind,1) = norm(PredData.preddatabin(ind,:));
        ActNorm(ind,1) = norm(TestSigsRightLength(ind,:));
    end
    vaf  = [vaf; calculateVAF(PredNorm,ActNorm)];  
