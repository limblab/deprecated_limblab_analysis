...
for i = 1:256    
    cors(i) = bayes_regression(theta(i));
end
...
hold on;
subplot(2,2,3),plot(theta, curve(theta), 'k--');
...
peak_th = find(cors == max(cors), 1);
[cor, X, N, N2, P, mdl] = bayes_regression(theta(peak_th));
...
function [cor, X, N, N2, P, mdl] = bayes_regression(th)
