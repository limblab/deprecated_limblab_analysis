function [data, sample_rate, active_channel_list] = run_stimplan(abort,avIN,EMG_labels,EMG_enable,handles,calMat,curr_action_text)
...
%% Run the stimulation
    ...
    if (strcmp(mode,'static_pulses')) || (strcmp(mode,'static_train'))
        ...
        % Run stimulation
        [data, sample_rate] = stimrec(abort, base_channel_amp, base_channel_pw, stim_freq, stim_pulses, rec_duration, mode, is_channel_active, EMG_labels, EMG_enable, nreps, stim_tip, freq_daq, stim_delay, handles, calMat, curr_action_text,active_channel_list,stagger_time);
        ...
        for i = 1:length(modulation_multipliers)
            ...
            if (strcmp(mode,'mod_amp'))
                ...
                for j = 1:length(ind)
                    ...
                    [data{i,j}, sample_rate] = stimrec(abort, pulseamps_IN, base_channel_pw_IN, stim_freq, stim_pulses, rec_duration, mode, is_channel_active, EMG_labels, EMG_enable, nreps, stim_tip, freq_daq, stim_delay_IN, handles, calMat, curr_action_text,active_channel_list,stagger_time);
