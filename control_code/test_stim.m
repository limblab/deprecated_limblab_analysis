...
chList = [1,2]; %which channels do I want to stimulate?
mode = ['static_pulses', 'static_pulses' ]; %can I do an array with different values here for different channels?
amp = [1,2]; % in mA
pw = [.2,.3]; % in ms
freq = 30; % in Hz
pulses = [1,1]; %how many happen? not sure ???
time2run = [0,0]; %this is never used ???
stim_tip = [.4, .4]; % interphase time in ms ???
...
for i=0:100 %stimulate a certain number of times
fopen(s); 
strOUT2 = fns_stim_prog('r',[0,1]); %check with line 165 of stimrec
fwrite(s,strOUT2);
fclose(s);
pause(0.1)
