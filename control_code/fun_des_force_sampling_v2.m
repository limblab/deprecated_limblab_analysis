function f = fun_des_force_sampling_v2(x,S,mF,mDir,fdes,stim,stdM,obsN,xMin,stdangle,c)
...
fXold = 0; fYold = 0; covarAll = zeros(2,2); covarAllObs = zeros(2,2); fvecG = zeros(length(x),2); covarAllDir = zeros(2,2);
for ii = 1:length(x)
    ...
    if ~sum(fvec)==0
        ...
        covarMuscDir = U*[stdangle(ii) 0; 0 0]*inv(U);
        
        % Store covariance matrix
        covarAll = covarAll + covarMusc; 
        covarAllObs = covarAllObs + covarMuscObs;
        covarAllDir = covarAllDir + covarMuscDir;
    end
end
% Add two sources of variability together
covarTot = covarAll + covarAllObs + covarAllDir;
