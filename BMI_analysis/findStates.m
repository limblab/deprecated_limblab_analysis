function [states,statemethods] = findStates(binnedData)
...
%     statemethods(x,1:10) = 'GFR thresh';
    ...
    states(:,2) = perf_bayes_clas(binnedData.spikeratedata,binsize,vel_magn);
    statemethods(2,1:14) = 'Complete Bayes';
    
    % 3- Classify states according to naive Bayesian using velocity peaks for training
    states(:,3) = peak_bayes_clas(binnedData.spikeratedata,binsize,vel_magn);
    ...
    states(:,4) = perf_LDA_clas(binnedData.spikeratedata,binsize,vel_magn);
    ...
    states(:,5) = peak_LDA_clas(binnedData.spikeratedata,binsize,vel_magn);
