function binned = bin_spikes(bdf)
%This function takes an input bdf and bins unit activity into 50 ms bins.
%Major chunk of code taken from Christian Ethier's "BMIDataAnalyzer" GUI
%M-file "convertBDF2binned.m"

out_struct = struct('spike_list',[], 'spike_rates', []);

%% Initialization

%Default settings
binsize = 0.05; %bin length in seconds (as are all time units). WILL POSSIBLY INCLUDE THIS AS INPUT IF CHANGE TO VARARGIN
start   = 0;   %default start time

if isfield(bdf,'emg')
        duration = double( bdf.emg.data(end,1) );
    elseif isfield(bdf,'force')
        duration = double( bdf.force.data(end,1) );
    elseif isfield(bdf,'pos')
        duration = double( bdf.pos(end,1) - bdf.pos(1,1) );
    else
        disp('bin_spikes: no emg or force field present in input')
end
        
stop     = floor(duration); %stop time
num_bins = (stop - start)/binsize;
num_bins = floor(num_bins);
min_rate = 0; %minimum firing rate. WILL POSSIBLY INCLUDE THIS AS INPUT IF CHANGE TO VARARGIN
%% Bin Spike Data
...
if ~isfield(bdf, 'units')
    ...
    spike_rates = [];
        ...
        end %closing "for i=1:total_num_units..."
...
end %closing "else" of "if ~isfield(bdf, 'units')..."
    
out_struct.spike_list  = spike_list;
out_struct.spike_rates = spike_rates;
